...
void Player::Update(unsigned long time_passed)
{
	...
	if (HasAura(64976)) // JUGGERNAUT (hack)
	{
		this->SendAuraUpdate(this->FindAura(64976)->m_auraSlot, false);
	}

	if (mstime >= m_AngerManagementTimer) // Hack. Needs to be handled in SpellAuras.cpp (Aura::SpellAuraModPowerRegen)
	{
		if (HasAura(12296))
		{
			this->ModPower(POWER_TYPE_RAGE, 10); // 1 rage every 3 seconds.
			m_AngerManagementTimer = mstime + 2000;
		}
	}

	if (mstime >= 6000 && HasAura(53302) && m_isMoving == false)
	{
		CastSpell(this, 64418, true);
	}

	if (mstime >= 6000 && HasAura(53303) && m_isMoving == false)
	{
		CastSpell(this, 64419, true);
	}

	if (mstime >= 6000 && HasAura(53304) && m_isMoving == false)
	{
		CastSpell(this, 64420, true);
	}
...
float Player::GetAverageItemLevel()
{
	...
	if(GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_BACK))
	back_IL = GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_BACK)->GetProto()->ItemLevel;
	else
	back_IL = 0;
	if (GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_HEAD))
		head_IL = GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_HEAD)->GetProto()->ItemLevel;
	else
		head_IL = 0;
	if (GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_CHEST))
		chest_IL = GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_CHEST)->GetProto()->ItemLevel;
	else
		chest_IL = 0;
	if (GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_NECK))
		neck_IL = GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_NECK)->GetProto()->ItemLevel;
	else
		neck_IL = 0;
	if (GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_SHOULDERS))
		shoulders_IL = GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_SHOULDERS)->GetProto()->ItemLevel;
	else
		shoulders_IL = 0;
	if (GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_WRISTS))
		wrist_IL = GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_WRISTS)->GetProto()->ItemLevel;
	else
		wrist_IL = 0;
	if (GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_HANDS))
		gloves_IL = GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_HANDS)->GetProto()->ItemLevel;
	else
		gloves_IL = 0;
	if (GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_WAIST))
		waist_IL = GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_WAIST)->GetProto()->ItemLevel;
	else
		waist_IL = 0;
	if (GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_LEGS))
		legs_IL = GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_LEGS)->GetProto()->ItemLevel;
	else
		legs_IL = 0;
	if (GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_FEET))
		boots_IL = GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_FEET)->GetProto()->ItemLevel;
	else
		boots_IL = 0;
	if (GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_FINGER1))
		ring1_IL = GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_FINGER1)->GetProto()->ItemLevel;
	else
		ring1_IL = 0;
	if (GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_FINGER2))
		ring2_IL = GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_FINGER2)->GetProto()->ItemLevel;
	else
		ring2_IL = 0;
	if (GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_TRINKET1))
		trinket1_IL = GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_TRINKET1)->GetProto()->ItemLevel;
	else
		trinket1_IL = 0;
	if (GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_TRINKET2))
		trinket2_IL = GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_TRINKET2)->GetProto()->ItemLevel;
	else
		trinket2_IL = 0;
	if (GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_MAINHAND))
		mainhand_IL = GetItemInterface()->GetInventoryItem(EQUIPMENT_SLOT_MAINHAND)->GetProto()->ItemLevel;
	else
		mainhand_IL = 0;



	sum = back_IL + head_IL + chest_IL + neck_IL + shoulders_IL + wrist_IL + gloves_IL + waist_IL + legs_IL + boots_IL + ring1_IL + ring2_IL + trinket1_IL + trinket2_IL + mainhand_IL;
...
void Player::KillPlayer()
{
	...
	sSpellHookInterface.OnDeath(this);
...
void Player::ProcessPendingUpdates()
{
    ...
    if (bCreationBuffer.size() || mOutOfRangeIdCount)
    {
        *(uint32*)&update_buffer[c] = ((mOutOfRangeIds.size() > 0) ? (mCreationCount + 1) : mCreationCount);
        ...
        if (c < (size_t)1000 || !CompressAndSendUpdateBuffer((uint32)c, update_buffer))
        {
...
void Player::OnWorldPortAck()
{
    ...
    if (pMapinfo)
    {
        ...
        if (pMapinfo->HasFlag(WMI_INSTANCE_WELCOME) && GetMapMgr())
        {
            ...
            welcome_msg += ". ";
            if (pMapinfo->type != INSTANCE_NONRAID && !(pMapinfo->type == INSTANCE_MULTIMODE && iInstanceType >= DUNGEON_HEROIC) && m_mapMgr->pInstance)
            {
...
void Player::EnableFrostwingAnticheat()
{
	if (this->IsInBg()) // If Player is in a battleground
	{
		if (this->GetMapMgr()->GetMapId() == 566) // Map: Eye of the Storm
		{
			if (this->GetPositionZ() >= 566) // If player is higher than this then player has to be cheating. BAN
			{
				sWorld.SendWorldText("[FrostwingAnticheat]: A player caught in Eye of the Storm flyhacking has been banned!");
				this->SetBanned("FrostwingAnticheat: Flyhacking");
				this->Kick();
			}
		}
		if (this->GetMapMgr()->GetMapId() == 30) // Map: Alterac Valley
		{
			if (this->GetPositionZ() >= 566) // If player is higher than this then player has to be cheating. BAN
			{
				sWorld.SendWorldText("[FrostwingAnticheat]: A player caught in Alterac Valley flyhacking has been banned!");
				this->SetBanned("FrostwingAnticheat: Flyhacking");
				this->Kick();
			}
		}
		if (this->GetMapMgr()->GetMapId() == 489) // Map: Warsong Gulch
		{
			if (this->GetPositionZ() >= 566) // If player is higher than this then player has to be cheating. BAN
			{
				sWorld.SendWorldText("[FrostwingAnticheat]: A player caught in Warsong Gulch flyhacking has been banned!");
				this->SetBanned("FrostwingAnticheat: Flyhacking");
				this->Kick();
			}
		}
		if (this->GetMapMgr()->GetMapId() == 529) // Map: Arathi Basin
		{
			if (this->GetPositionZ() >= 566) // If player is higher than this then player has to be cheating. BAN
			{
				sWorld.SendWorldText("[FrostwingAnticheat]: A player caught in Arathi Basin flyhacking has been banned!");
				this->SetBanned("FrostwingAnticheat: Flyhacking");
				this->Kick();
			}
		}
		if (this->GetMapMgr()->GetMapId() == 628) // Map: Isle of Conquest
		{
			if (this->GetPositionZ() >= 566) // If player is higher than this then player has to be cheating. BAN
			{
				sWorld.SendWorldText("[FrostwingAnticheat]: A player caught in Isle of Conquest flyhacking has been banned!");
				this->SetBanned("FrostwingAnticheat: Flyhacking");
				this->Kick();
			}
		}
		if (this->GetMapMgr()->GetMapId() == 607) // Map: Strand of the Ancients
		{
			if (this->GetPositionZ() >= 566) // If player is higher than this then player has to be cheating. BAN
			{
				sWorld.SendWorldText("[FrostwingAnticheat]: A player caught in Strand of Ancients flyhacking has been banned!");
				this->SetBanned("FrostwingAnticheat: Flyhacking");
				this->Kick();
			}
		}
...
void Player::Insert_DB_NewRune(uint32 playerGUID, uint32 cinderglacier, uint32 lichbane, uint32 spellbreaking, uint32 spellshattering, uint32 swordbreaking, uint32 swordshattering, uint32 nerubiancarapace, uint32 razorice, uint32 fallencrusader, uint32 stoneskin)
{
	CharacterDatabase.Execute("DELETE FROM rune_weapon_status_dk WHERE guid = %u", playerGUID); // Delete player entry if any first to prevent errors and misreads.
	CharacterDatabase.Execute("INSERT INTO rune_weapon_status_dk VALUES(%u, %u, %u, %u, %u, %u, %u, %u, %u, %u, %u)", playerGUID, cinderglacier, lichbane, spellbreaking, spellshattering, swordbreaking, swordshattering, nerubiancarapace, razorice, fallencrusader, stoneskin);
}

bool Player::Has_Rune_Weapon_Enchantment_cinderglacier(uint32 playerGUID)
{
	QueryResult* result = CharacterDatabase.Query("SELECT guid,cinderglacier FROM rune_weapon_status_dk WHERE guid = %u and cinderglacier = 1", playerGUID);

	if (result) // Player Has cinderglacier
	{
		return true;
	}
	else
...
bool Player::Has_Rune_Weapon_Enchantment_fallencrusader(uint32 playerGUID)
{
	QueryResult* result = CharacterDatabase.Query("SELECT guid,cinderglacier FROM rune_weapon_status_dk WHERE guid = %u and fallencrusader = 1", playerGUID);

	if (result) // Player Has fallen crusader
	{
		return true;
	}
	else
...
void Player::DealDamage(Unit* pVictim, uint32 damage, uint32 targetEvent, uint32 unitEvent, uint32 spellId, bool no_remove_auras)
{
	...
	/////////////////////////////////////////////////SPELL PROC TEMP//////////////////////////////////////////////////
	uint32 proc_Chance = RandomUInt(0, 100);
	...
	//Death Knight: Rune of Cinderglacier
	...
	if (Has_Rune_Weapon_Enchantment_cinderglacier(this->GetGUID()) == true && proc_Chance <= 8)
	{
		this->CastSpell(this, 53386, true);
	}

	//Death Knight: Rune of Fallen Crusader
	//Affixes your rune weapon with a rune that has a chance to heal you for 3% and increase total Strength by 15% for 15 sec.
	if (Has_Rune_Weapon_Enchantment_fallencrusader(this->GetGUID()) == true && proc_Chance <= 8) //proc_Chance <= 6
	{
		this->CastSpell(this, 53365, true);
	}
    ...
    if (pVictim->GetHealth() <= damage)
    {
		Creature* creatureScript = TO_CREATURE(pVictim);
		if (pVictim->IsCreature() && creatureScript->IsKilledByScript == true) // This is killed by a script and or despawned.
		{
			pVictim->SetHealth(1);
			return;
		}
        ...
        if (pVictim->IsPlayer())
        {
			...
			sSpellHookInterface.OnKillPlayer(this, playerVictim);
