...
void WorldSession::HandleLfgJoinOpcode(WorldPacket& recv_data)
{
	...
	// Handles Item Level Requirement
	// Needs alot of improvement like REAL LOCKING!!!
	for (std::set<uint32>::iterator it = newDungeons.begin(); it != newDungeons.end(); ++it)
	{
		DBC::Structures::LFGDungeonEntry const* dungeon_id = sLFGDungeonStore.LookupEntry(*it);
		if (dungeon_id && dungeon_id->ID == 262 && GetPlayer()->GetAverageItemLevel() < 160) // Random Heroic WOTLK dungeons require minimum item level of 180 to queue
		{
			GetPlayer()->SendChatMessage(CHAT_MSG_SYSTEM, 0, "You do not meet the requirements for the chosen dungeons");
			GetPlayer()->SendChatMessage(CHAT_MSG_SYSTEM, 0, "(Item level is too low)"); // Custom remove for blizzlike purposes
			return;
		}
	}
...
void WorldSession::HandleLfgPlayerLockInfoRequestOpcode(WorldPacket& recv_data)
{
	...
	for (LfgDungeonSet::const_iterator it = randomDungeons.begin(); it != randomDungeons.end(); ++it)
	{
		...
		if (reward)
		{
			qRew = QuestStorage.LookupEntry(reward->firstQuest);
			if (qRew)
			{
				done = GetPlayer()->HasFinishedQuest(qRew->id);
				if (done)
					qRew = QuestStorage.LookupEntry(reward->otherQuest);
			}
		}
		if (qRew)
		{
			...
			for (uint8 i = 0; i < 4; ++i)
				...
				{
					ItemPrototype const* item = ItemPrototypeStorage.LookupEntry(qRew->reward_item[i]);
...
void WorldSession::SendLfgUpdateParty(const LfgUpdateData& updateData, uint64 state)
{
	bool join = false;
	bool extrainfo = false;
	bool queued = false;
...
void WorldSession::SendLfgPlayerReward(uint32 rdungeonEntry, uint32 sdungeonEntry, uint8 done, const LfgReward* reward, const Quest* qRew)
{
	...
	uint8 itemNum = uint8(qRew ? qRew->GetRewardItemCount() : 0);

	Log.Debug("LfgHandler", "SMSG_LFG_PLAYER_REWARD %u rdungeonEntry: %u - sdungeonEntry: %u - done: %u", GetPlayer()->GetGUID(), rdungeonEntry, rdungeonEntry, done);
	...
	if (itemNum)
	{
		for (uint8 i = 0; i < 4; ++i)
		{
			...
			ItemPrototype * iProto = ItemPrototypeStorage.LookupEntry(qRew->reward_item[i]);
