...
uint8 Spell::DidHit(uint32 effindex, Unit* target)
{
	...
	/* Check if unit target has immune from spellId */
	if (u_victim->IsCreature() && u_victim->m_spellIdImmune == GetProto()->Id) // if casted spell equals the immuned spell id then the creature should be immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}

	/* Check if unit target has immune from addImmunity */
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[1] == 1 && GetProto()->MechanicsType == MECHANIC_CHARMED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[2] == 1 && GetProto()->MechanicsType == MECHANIC_DISORIENTED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[3] == 1 && GetProto()->MechanicsType == MECHANIC_DISARMED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[4] == 1 && GetProto()->MechanicsType == MECHANIC_DISTRACED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[5] == 1 && GetProto()->MechanicsType == MECHANIC_FLEEING) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[6] == 1 && GetProto()->MechanicsType == MECHANIC_CLUMSY) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[7] == 1 && GetProto()->MechanicsType == MECHANIC_ROOTED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[8] == 1 && GetProto()->MechanicsType == MECHANIC_PACIFIED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[9] == 1 && GetProto()->MechanicsType == MECHANIC_SILENCED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[10] == 1 && GetProto()->MechanicsType == MECHANIC_ASLEEP) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[11] == 1 && GetProto()->MechanicsType == MECHANIC_ENSNARED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[12] == 1 && GetProto()->MechanicsType == MECHANIC_STUNNED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[13] == 1 && GetProto()->MechanicsType == MECHANIC_FROZEN) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[14] == 1 && GetProto()->MechanicsType == MECHANIC_INCAPACIPATED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[15] == 1 && GetProto()->MechanicsType == MECHANIC_BLEEDING) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[16] == 1 && GetProto()->MechanicsType == MECHANIC_HEALING) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[17] == 1 && GetProto()->MechanicsType == MECHANIC_POLYMORPHED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[18] == 1 && GetProto()->MechanicsType == MECHANIC_BANISHED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[19] == 1 && GetProto()->MechanicsType == MECHANIC_SHIELDED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[20] == 1 && GetProto()->MechanicsType == MECHANIC_SHACKLED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[21] == 1 && GetProto()->MechanicsType == MECHANIC_MOUNTED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[22] == 1 && GetProto()->MechanicsType == MECHANIC_SEDUCED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[23] == 1 && GetProto()->MechanicsType == MECHANIC_TURNED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[24] == 1 && GetProto()->MechanicsType == MECHANIC_HORRIFIED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[25] == 1 && GetProto()->MechanicsType == MECHANIC_INVULNARABLE) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[26] == 1 && GetProto()->MechanicsType == MECHANIC_INTERRUPTED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[27] == 1 && GetProto()->MechanicsType == MECHANIC_DAZED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[28] == 1 && GetProto()->MechanicsType == MECHANIC_DISCOVERY) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[29] == 1 && GetProto()->MechanicsType == MECHANIC_INVULNERABLE) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[30] == 1 && GetProto()->MechanicsType == MECHANIC_SAPPED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
	if (u_victim->IsCreature() && u_victim->m_immuneSlots[31] == 1 && GetProto()->MechanicsType == MECHANIC_ENRAGED) // Spell contains charmed mechanic but we are immune to it.
	{
		return SPELL_DID_HIT_IMMUNE;
	}
...
uint8 Spell::prepare(SpellCastTargets* targets)
{
    ...
    if (p_caster != NULL)
    {
		...
		if (!sSpellHookInterface.OnCastSpell(p_caster, GetProto(), this))
		{
			DecRef();
			return SPELL_FAILED_UNKNOWN;
		}
	...
	if (p_caster != NULL && p_caster->HasAura(64976) && GetProto()->Id == 100 || GetProto()->Id == 6178 || GetProto()->Id == 11578)
	{
		cancastresult = SPELL_CANCAST_OK;
	}
	else
...
void Spell::cast(bool check)
{
	...
	if (GetProto()->override_cast_check == true) // Check for override cast checks and automaticlly return okay.
	{
		cancastresult = SPELL_CANCAST_OK;
	}
	else
    ...
    if (cancastresult == SPELL_CANCAST_OK)
    {
		...
		if (GetProto()->override_cast_check == true)
		{
			SendCastResult(SPELL_CANCAST_OK);
		}
		else
		{
			SendCastResult(cancastresult);
        ...
        if (!(hasAttribute(ATTRIBUTE_ON_NEXT_ATTACK) && !m_triggeredSpell))  //on next attack
        {
            ...
            else//not handled in this switch
            {
...
void Spell::finish(bool successful)
{
    ...
    if (u_caster != NULL)
    {
		...
		{
			sSpellHookInterface.OnAfterCastSpell(TO_PLAYER(u_caster), GetProto(), this);
		}
        CALL_SCRIPT_EVENT(u_caster, OnCastSpell)(GetProto()->Id);

        u_caster->m_canMove = true;

		if (u_caster->IsCreature() && u_caster->isRooted_Moonscript()) //Cronic: Do not remove this fixes issues with creatures interrupting spell casts as soon as they are casted.
		{
			u_caster->Unroot_Moonscript();
		}
    ...
    if (p_caster != NULL)
    {
		...
		if (GetProto()->Id == 1715)
		{
			...
			if (u_caster != nullptr)
			{
				...
				if (procChance <= 5 && p_caster->HasSpell(12289) || p_caster->HasAura(12289))
				{
					p_caster->CastSpell(p_caster->GetMapMgr()->GetUnit(p_caster->GetSelection()), 23694, false);
				}
				if (procChance <= 10 && p_caster->HasSpell(12668) || p_caster->HasAura(12668))
				{
					p_caster->CastSpell(p_caster->GetMapMgr()->GetUnit(p_caster->GetSelection()), 23694, false);
				}
				if (procChance <= 15 && p_caster->HasSpell(23695) || p_caster->HasAura(23695))
				{
					p_caster->CastSpell(p_caster->GetMapMgr()->GetUnit(p_caster->GetSelection()), 23694, false);
				}
			}
...
/*void Spell::SendSpellStart()
{
	...
	if (p_caster && p_caster->getClass() == DEATHKNIGHT && GetProto()->runeCostID)
	{
		cast_flags |= SPELL_GO_FLAGS_ITEM_CASTER;
	...
	if (cast_flags & SPELL_GO_FLAGS_POWER_UPDATE)
		data << uint32(u_caster->GetPower(GetProto()->powerType));

	if (cast_flags & SPELL_GO_FLAGS_RUNE_UPDATE) //send new runes
	{
...
void Spell::SendSpellStart()
{
	...
	if (cast_flags & SPELL_GO_FLAGS_RANGED)
	{
		ItemPrototype* ip = NULL;
		if (GetProto()->Id == SPELL_RANGED_THROW) // throw
		{
...
void Spell::SendSpellGo()
{
	...
	ItemPrototype* ip = NULL;
	uint32 cast_flags = 0;
	if (m_triggeredSpell && m_triggeredByAura && !(hasAttributeExB(ATTRIBUTESEXB_ACTIVATE_AUTO_SHOT)))
		cast_flags |= SPELL_GO_FLAGS_LOCK_PLAYER_CAST_ANIM;

	if (GetType() == SPELL_DMG_TYPE_RANGED)
		cast_flags |= SPELL_GO_FLAGS_RANGED; // 0x20 RANGED

	if (GetProto()->powerType > 0)
		if (GetProto()->powerType != POWER_TYPE_HEALTH) // 0x
			cast_flags |= SPELL_GO_FLAGS_POWER_UPDATE;
	...
	if (ModeratedTargets.size() > 0)
		cast_flags |= SPELL_GO_FLAGS_EXTRA_MESSAGE;
...
uint8 Spell::CanCast(bool tolerate)
{
	...
	bool bypass = (p_caster && (p_caster->m_bypassCastRequirements[0] & GetProto()->SpellGroupType[0] ||
		p_caster && p_caster->m_bypassCastRequirements[1] & GetProto()->SpellGroupType[1] ||
		p_caster && p_caster->m_bypassCastRequirements[2] & GetProto()->SpellGroupType[2])); // related to aura 262
    ...
    if (u_caster)
    {
        ...
        {
            // Warbringer/ Juggernaut(Warrior 51Prot Talent effect)
            if ((GetProto()->Id != 100 && GetProto()->Id != 6178 && GetProto()->Id != 11578)
                || (p_caster != NULL && !bypass))
    ...
    if (p_caster)
    {
         ...
         */
		if (!(p_caster->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NO_REAGANT_COST) && hasAttributeExE(FLAGS6_REAGENT_REMOVAL) && p_caster->HasAura(63165) || p_caster->HasAura(63167)))
        {
            // Skip this with enchanting scrolls and if caster has aura
		...
		{
			if (!p_caster->HasFlag(UNIT_FIELD_AURASTATE, 1 << (GetProto()->CasterAuraState - 1)))
    ...
    if (m_targets.m_unitTarget)
    {
        ...
        if (target)
        {
            ...
            if (p_caster != NULL)
            {
                ...
                // check aurastate
                if (GetProto()->TargetAuraState && !bypass)
                {
					if (!target->HasFlag(UNIT_FIELD_AURASTATE, 1 << (GetProto()->TargetAuraState - 1)))
					{
...
int32 Spell::DoCalculateEffect(uint32 i, Unit* target, int32 value)
{
	...
	sSpellHookInterface.OnCalculateEffect(this, GetProto()->Id, i, target, value);

	if (GetProto()->ap_override != 0) // Final Check
	{
		if (p_caster != NULL && p_caster->getClass() != HUNTER) // if class is not a hunter
			value += (p_caster->GetAP() * GetProto()->ap_override);
		if (p_caster != NULL && p_caster->getClass() == HUNTER)
			value += (p_caster->GetRAP() * GetProto()->ap_override);
	}
	if (GetProto()->ap_dot_override != 0) // Final Check
	{
		if (p_caster != NULL && p_caster->getClass() != HUNTER) // if class is not a hunter
			value += (p_caster->GetAP() * GetProto()->ap_dot_override);
		if (p_caster != NULL && p_caster->getClass() == HUNTER)
			value += (p_caster->GetRAP() * GetProto()->ap_dot_override);
	}
