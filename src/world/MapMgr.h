...
#define IS_PERSISTENT_INSTANCE(p) (((p)->m_mapInfo->type == INSTANCE_MULTIMODE && (p)->m_difficulty >= DUNGEON_HEROIC) || (p)->m_mapInfo->type == INSTANCE_RAID)
#define IS_RESETABLE_INSTANCE(p) (!(p)->m_persistent && ((p)->m_mapInfo->type == INSTANCE_NONRAID || ((p)->m_mapInfo->type == INSTANCE_MULTIMODE && (p)->m_difficulty == DUNGEON_NORMAL)))
...
class SERVER_DECL MapMgr : public CellHandler <MapCell>, public EventableObject, public CThread, public WorldStatesHandler::WorldStatesObserver
{
    ...
    public:
		...
		void LoadAllCells();
		void BanCheatersByInstanceID(int32 InstanceID);
		void SendAchievementToAllPlayers(int32 AchID, int32 InstanceID);
		void SendAchievementToAllPlayers(int32 AchID);
        ...
        void RemoveAuraFromGroup(uint32 spellId, uint32 mapId);
