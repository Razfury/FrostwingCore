...
pSpellAura SpellAuraHandler[TOTAL_SPELL_AURAS] =
{
    ...
    &Aura::SpellAuraRemoveReagentCost,                                      // 256 Remove reagent cost (SPELL_AURA_REMOVE_REAGANT_COST)
...
const char* SpellAuraNames[TOTAL_SPELL_AURAS] =
{
    ...
    "",                                                 // 214
    "SPELL_AURA_REMOVE_REAGANT_COST",                   // 215
    ...
    "SPELL_AURA_REMOVE_REAGANT_COST",                   // 256
    "",                                                 // 257
    "",                                                 // 258
    "",                                                 // 259
    "",                                                 // 260
...
void Aura::Remove()
{
	...
	sSpellHookInterface.OnAuraRemove(this);
    ...
    if (apply)
    {
		...
		case 12721: // Deep Wounds
			if (c != NULL)
			{
				if (c->HasAura(12834))
				{
					dmg = 0; // set damage to zero as we are going to recalculate the real damage
					dmg = (CalculateDamage(c, GetTarget(), MELEE, 0, dbcSpell.LookupEntry(12721)) * 0.14) + RandomUInt(1, 4); // 16% of a average melee attack + the random feel :D
				}
				if (c->HasAura(12849))
				{
					dmg = 0; // set damage to zero as we are going to recalculate the real damage
					dmg = (CalculateDamage(c, GetTarget(), MELEE, 0, dbcSpell.LookupEntry(12721)) * 0.30) + RandomUInt(1, 4); // 32% of a average melee attack + the random feel :D
				}
				if (c->HasAura(12867))
				{
					dmg = 0; // set damage to zero as we are going to recalculate the real damage
					dmg = (CalculateDamage(c, GetTarget(), MELEE, 0, dbcSpell.LookupEntry(12721)) * 0.46) + RandomUInt(1, 4); // 48% of a average melee attack + the random feel :D
				}
			}
			break;
		case 50536:
			if (c != NULL)
				dmg = 0; // set damage to zero as we are going to recalculate the real damage
				dmg = (RandomUInt(167, 443) + c->GetAP()*0.30) / 10;
			break;
			// Death Knight Spells
            case 703:
            case 8631:
		...
		if (m_spellProto->ap_override != 0) // Final Check to see if entry has a DB override
		{
			if (c != NULL && c->getClass() != HUNTER) // if class is not a hunter
				dmg = mod->m_amount;
			dmg += (c->GetAP() * m_spellProto->ap_override);
			if (c != NULL && c->getClass() == HUNTER)
				dmg = mod->m_amount;
			dmg += (c->GetRAP() * m_spellProto->ap_override);
		}
		if (m_spellProto->ap_dot_override != 0) // Final Check
		{
			if (c != NULL && c->getClass() != HUNTER) // if class is not a hunter
				dmg = mod->m_amount;
			dmg += (c->GetAP() * m_spellProto->ap_dot_override);
			if (c != NULL && c->getClass() == HUNTER)
				dmg = mod->m_amount;
			dmg += (c->GetRAP() * m_spellProto->ap_dot_override);
		}
...
{
	sSpellHookInterface.OnSpellAura(this, GetSpellId(), apply, SPELL_AURA_DUMMY);
	...
	Player *p_caster = m_caster ? (m_caster->IsPlayer() ? TO_PLAYER(m_caster) : nullptr) : nullptr;
...
void Aura::SpellAuraModStun(bool apply)
{
    if (apply)
    {
        ...
        m_target->m_rooted++;
		m_target->m_rooted_moonscript++;

        if (m_target->m_rooted == 1)
            m_target->Root();

		if (m_target->m_rooted_moonscript == 1)
			m_target->Root_Moonscript();
    ...
    else if ((m_flags & (1 << mod->i)) == 0)   //add these checks to mods where immunity can cancel only 1 mod and not whole spell
    {
		...
		if (m_target->m_rooted_moonscript == 0)
			m_target->Unroot_Moonscript();
...
{
    SetPositive();
    if (m_spellProto->Effect[mod->i] == SPELL_EFFECT_APPLY_FRIEND_AREA_AURA)  ///\todo WTF is this crap? TODO clean this
        return;
...
void Aura::SpellAuraPeriodicTriggerSpell(bool apply)
{
    ...
    */
	...
	if (m_spellProto->Id == 66)
	{
		if (!GetUnitCaster()->HasAura(31574) || !GetUnitCaster()->HasAura(31575) || !GetUnitCaster()->HasAura(54354)) // Has no prismatic cloak ranks
		{
			m_spellProto->EffectAmplitude[2] = 3000;
		}
		if (GetUnitCaster()->HasAura(31574))
		{
			m_spellProto->EffectAmplitude[2] = 2000;
		}
		if (GetUnitCaster()->HasAura(31575))
		{
			m_spellProto->EffectAmplitude[2] = 1000;
		}
		if (GetUnitCaster()->HasAura(54354))
		{
			m_spellProto->EffectAmplitude[2] = 1;
		}
	}
    ...
    if (apply)
    {
		...
		switch (GetSpellProto()->Id)
		{
			case 48294: // King Ymiron - Bane (Utgarde Pinnacle)
			{
				...
				Unit * c = GetUnitCaster();
				m_target->AddProcTriggerSpell(triggerspell, 48294, m_casterGuid, 100, PROC_ON_MELEE_ATTACK_VICTIM, 0, groupRelation, NULL); // "whenever the caster is dealt direct damage"
				return;
			}break;
		}
...
void Aura::SpellAuraTransform(bool apply)
{
	...
	sSpellHookInterface.OnSpellAura(this, GetSpellId(), apply, SPELL_AURA_TRANSFORM);
    if (sScriptMgr.CallScriptedDummyAura(GetSpellId(), mod->i, this, apply))
        return;
...
{
	sSpellHookInterface.OnSpellAura(this, GetSpellProto()->Id, apply, SPELL_AURA_PERIODIC_TRIGGER_DUMMY);
    ...
    if (apply)
    {
		switch (GetSpellProto()->Id)
		{
			case 68870: // Soulstorm
			{
				...
				GetTarget()->CastSpell(GetTarget(), SoulstormVisualSpells[RandomUInt(1, 8)], true);
				return;
			}break;
		}
...
void Aura::SpellAuraAddPctMod(bool apply)
{
	...
	sSpellHookInterface.OnSpellAura(this, GetSpellProto()->Id, apply, SPELL_AURA_ADD_PCT_MODIFIER);
...
void Aura::SpellAuraIgnoreTargetAuraState(bool apply)
{
	...
	{
		for (uint32 x = 0; x<3; x++)
			TO_PLAYER(m_target)->m_bypassCastRequirements[x] |= m_spellProto->EffectSpellClassMask[mod->i][x];
	}
	else
		for (uint32 x = 0; x<3; x++)
			TO_PLAYER(m_target)->m_bypassCastRequirements[x] &= ~m_spellProto->EffectSpellClassMask[mod->i][x];
