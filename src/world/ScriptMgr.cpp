...
initialiseSingleton(SpellHookInterface);
...
void ScriptMgr::LoadScripts()
{
	...
	if (SpellHookInterface::getSingletonPtr() == NULL)
		new SpellHookInterface;
...
void ScriptMgr::UnloadScripts()
{
	...
	if (SpellHookInterface::getSingletonPtr())
		delete SpellHookInterface::getSingletonPtr();
...
void ScriptMgr::RegisterSpellType(SpellHookEvents event, void* function_pointer)
{
	ARCEMU_ASSERT(event < NUM_SPELL_HOOKS);
	_spellhooks[event].insert(function_pointer);
}
...
bool ScriptMgr::has_spell_hook(SpellHookEvents evt, void* ptr) const
{
	return (_spellhooks[evt].size() != 0 && _spellhooks[evt].find(ptr) != _spellhooks[evt].end());
}
...
/* SpellHook Implementations */
void SpellHookInterface::OnKillPlayer(Player* pPlayer, Player* pVictim)
{
	SpellHookList spellhookList = sScriptMgr._spellhooks[SPELL_EVENT_KILL_PLAYER];
	for (SpellHookList::iterator itr = spellhookList.begin(); itr != spellhookList.end(); ++itr)
		((shOnKillPlayer)*itr)(pPlayer, pVictim);
}

void SpellHookInterface::OnDeath(Player* pPlayer)
{
	SpellHookList spellhookList = sScriptMgr._spellhooks[SPELL_EVENT_DEATH];
	for (SpellHookList::iterator itr = spellhookList.begin(); itr != spellhookList.end(); ++itr)
		((shOnDeath)*itr)(pPlayer);
}

bool SpellHookInterface::OnAfterCastSpell(Player* pPlayer, SpellEntry* pSpell, Spell* spell)
{
	SpellHookList spellhookList = sScriptMgr._spellhooks[SPELL_EVENT_AFTER_CAST_SPELL];
	bool ret_val = true;
	for (SpellHookList::iterator itr = spellhookList.begin(); itr != spellhookList.end(); ++itr)
	{
		bool rv = ((shOnAfterCastSpell)* itr)(pPlayer, pSpell, spell);
...
bool SpellHookInterface::OnCastSpell(Player* pPlayer, SpellEntry* pSpell, Spell* spell)
{
	SpellHookList spellhookList = sScriptMgr._spellhooks[SPELL_EVENT_CAST_SPELL];
	bool ret_val = true;
	for (SpellHookList::iterator itr = spellhookList.begin(); itr != spellhookList.end(); ++itr)
	{
		bool rv = ((shOnCastSpell)* itr)(pPlayer, pSpell, spell);
		if (rv == false)  // never set ret_val back to true, once it's false
			ret_val = false;
	}
	return ret_val;
...
void SpellHookInterface::OnCritAttack(Player* pPlayer)
{
	SpellHookList spellhookList = sScriptMgr._spellhooks[SPELL_EVENT_ON_CRIT_ATTACK];
	for (SpellHookList::iterator itr = spellhookList.begin(); itr != spellhookList.end(); ++itr)
		((shOnCritAttack)*itr)(pPlayer);
}

void SpellHookInterface::OnCriticalAttack(Player* pPlayer, uint32 spellId)
{
	SpellHookList spellhookList = sScriptMgr._spellhooks[SPELL_EVENT_ON_CRITICAL_ATTACK];
	for (SpellHookList::iterator itr = spellhookList.begin(); itr != spellhookList.end(); ++itr)
		((shOnCriticalAttack)*itr)(pPlayer, spellId);
}

void SpellHookInterface::OnAuraRemove(Aura* aura)
{
	SpellHookList spellhookList = sScriptMgr._spellhooks[SPELL_EVENT_AURA_REMOVE];
	for (SpellHookList::iterator itr = spellhookList.begin(); itr != spellhookList.end(); ++itr)
		((shOnAuraRemove)*itr)(aura);
}

int32 SpellHookInterface::OnCalculateEffect(Spell* pSpell, uint32 spellId, uint32 i, Unit* target, int32 value)
{
	SpellHookList spellhookList = sScriptMgr._spellhooks[SPELL_EVENT_CALCULATE_EFFECT];
	for (SpellHookList::iterator itr = spellhookList.begin(); itr != spellhookList.end(); ++itr)
		((shOnCalculateEffect)*itr)(pSpell, spellId, i, target, value);
	
	return value;
}

bool SpellHookInterface::OnResurrect(Player* pPlayer)
{
	SpellHookList spellhookList = sScriptMgr._spellhooks[SPELL_EVENT_RESURRECT];
	bool ret_val = true;
	for (SpellHookList::iterator itr = spellhookList.begin(); itr != spellhookList.end(); ++itr)
	{
		bool rv = ((shOnResurrect)* itr)(pPlayer);
		if (rv == false)  // never set ret_val back to true, once it's false
			ret_val = false;
	}
	return ret_val;
...
void SpellHookInterface::OnSpellEffect(Spell* sp, Unit* spellcaster, uint32 spellId, uint32 SpellEffectType)
{
	SpellHookList spellhookList = sScriptMgr._spellhooks[SPELL_EVENT_EFFECT];
	for (SpellHookList::iterator itr = spellhookList.begin(); itr != spellhookList.end(); ++itr)
		((shOnSpellEffect)*itr)(sp, spellcaster, spellId, SpellEffectType);
}

void SpellHookInterface::OnSpellAura(Aura* aur, uint32 spellId, bool apply, uint32 SpellAuraType)
{
	SpellHookList spellhookList = sScriptMgr._spellhooks[SPELL_EVENT_AURA];
	for (SpellHookList::iterator itr = spellhookList.begin(); itr != spellhookList.end(); ++itr)
		((shOnSpellAura)*itr)(aur, spellId, apply, SpellAuraType);
}
