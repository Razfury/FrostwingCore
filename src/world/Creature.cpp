...
float CalcHPCoefficient(MapInfo* mi, uint32 mode, bool boss)
{
    ...
    if (mi->type == INSTANCE_RAID)
    {
        ...
        if (boss == true)
        {
            ...
            switch (mode)
            {
                case RAID_10MAN_NORMAL:
                    coeff = 1.0f;
                    break;

                case RAID_10MAN_HEROIC:
                    coeff = 1.25f;
                    break;

                case RAID_25MAN_NORMAL:
                    if (hasheroic)
                        coeff = 5.0f;
                    else
                        coeff = 3.0f;
...
void Creature::InstanceCreatureAwardEmblem(Creature* u) // Checks are made to see if the dungeon is heroic or not.
{
...
void Creature::InstanceCreatureAwardAchievement(Creature* u, uint32 achid)
{
	for (PlayerStorageMap::iterator itr = u->GetMapMgr()->m_PlayerStorage.begin(); itr != u->GetMapMgr()->m_PlayerStorage.end(); ++itr)
	{
...
Player* Creature::GetPlayersInInstance(Creature* u) // Checks are made to see if the dungeon is heroic or not.
{
	for (PlayerStorageMap::iterator itr = u->GetMapMgr()->m_PlayerStorage.begin(); itr != u->GetMapMgr()->m_PlayerStorage.end(); ++itr)
	{
...
{
	if (HasAura(72178) || HasAura(70482))
	{
		return;
	}
	if (!HasAura(72178) || HasAura(70482))
...
bool Creature::Load(CreatureSpawn* spawn, uint32 mode, MapInfo* info)
{
	...
	if (isattackable(spawn) && !(proto->isTrainingDummy)) // Fix for creature vehicles if vehicle has energy then assume its a vehicle if not then assume its a creature.
	{
	...
	if (isattackable(spawn) && IsVehicle() && GetPowerType() == POWER_TYPE_ENERGY) // If it is a vehicle and it has energy set allowed combat to false
	{
		GetAIInterface()->SetAllowedToEnterCombat(false);
		GetAIInterface()->SetAIType(AITYPE_PASSIVE);
	}
	if (isattackable(spawn) && IsVehicle() && GetPowerType() != POWER_TYPE_ENERGY)
	{
		GetAIInterface()->SetAllowedToEnterCombat(true);
	}
...
void Creature::Load(CreatureProto* proto_, float x, float y, float z, float o)
{
	...
	if (IsVehicle() && GetPowerType() == POWER_TYPE_ENERGY) // If it is a vehicle and it has energy set allowed combat to false
	{
		GetAIInterface()->SetAllowedToEnterCombat(false);
		GetAIInterface()->SetAIType(AITYPE_PASSIVE);
	}
...
{
	//Player* p22 = TO< Player* >(GetPlayerOwner());
    ...
    {
		if (pVictim->GetEntry() == 23970) // Do not kill units that are killed via a script.
		{
			damage = 0;
			pVictim->SetHealth(1); // Set health to 1.
			return;
		}
		...
		if (pVictim->GetEntry() == 29307) // Drakkari Colossus (Killed via script - blizzlike)
		{
			pVictim->SetHealth(1);
			return;
		}

		if (this->IsVehicle() && GetPowerType() == POWER_TYPE_ENERGY) // CRONIC: DO NOT REMOVE THIS GIVES VEHICLES QUEST CREDIT TO RIDERS.
		{
...
void Creature::SetRoot()
{
	CastSpell(this, 18373, true);
}

void Creature::UnsetRoot()
{
	RemoveAura(18373);
}
...
{
	if (IsKilledByScript == true) // This is killed by a script and or despawned.
	{
		SetHealth(1);
		return;
	}
	...
	if (HasAura(27243)) // Seed of Corruption rank 1
	{
		Unit* owner = GetMapMgr()->GetPlayer(owner->GetGUID());
		owner->CastSpellAoF(GetPositionX(), GetPositionY(), GetPositionZ(), dbcSpell.LookupEntryForced(27285), true);
		owner->CastSpell(pAttacker, 37826, true);
	}

	if (HasAura(47836)) // Seed of Corruption rank 2
	{
		Unit* owner = GetMapMgr()->GetPlayer(owner->GetGUID());
		owner->CastSpellAoF(GetPositionX(), GetPositionY(), GetPositionZ(), dbcSpell.LookupEntryForced(47834), true);
		owner->CastSpell(pAttacker, 37826, true);
	}

	if (HasAura(47835)) // Seed of Corruption rank 3
	{
		Unit* owner = GetMapMgr()->GetPlayer(owner->GetGUID());
		owner->CastSpellAoF(GetPositionX(), GetPositionY(), GetPositionZ(), dbcSpell.LookupEntryForced(47833), true);
		owner->CastSpell(pAttacker, 37826, true);
	}
