...
class RabidThistleBear : public HybridBossScriptAI //Quest:2118
{
	...
	{
		HasSpawned = false;
		//RegisterAIUpdateEvent(1000); - Unneeded at this point whenever the bear goes into combat it will check for it.
		ParentClass::OnLoad();
	}

	void AIUpdate()
	{
		if (_unit->GetMapMgr()->GetInterface()->GetGameObjectNearestCoords(_unit->GetPositionX(), _unit->GetPositionY(), _unit->GetPositionZ(), 111148) && HasSpawned == false) // Night elf bear trap
		{
			HasSpawned = true;
			SpawnCreature(11836, false); // Captured thistle bear.
			sEAS.KillMobForQuest(GetNearestPlayer(), 2118, 0);
			_unit->Despawn(1000, 60000);
		}
...
class CRabidThistleBear : public HybridBossScriptAI //Quest:2118 (Captured)
{
	...
	void OnLoad()
	{
		_unit->GetAIInterface()->SetUnitToFollow(GetNearestPlayer());
		_unit->SetFaction(35);
...
void SetupUnsorted(ScriptMgr* mgr)
{
	...
	mgr->register_creature_script(2164, &RabidThistleBear::Create);
	mgr->register_creature_script(11836, &CRabidThistleBear::Create);
}
