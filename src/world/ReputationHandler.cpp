...
void Player::ModStanding(uint32 Faction, int32 Value)
{
    ...
    if ((GetMapMgr()->GetMapInfo()->minlevel == 80 || (GetMapMgr()->iInstanceMode == DUNGEON_HEROIC && GetMapMgr()->GetMapInfo()->minlevel_heroic == 80)) && ChampioningFactionID != 0)
...
void Player::Reputation_OnKilledUnit(Unit* pUnit, bool InnerLoop)
{
    ...
    if (modifier != 0)
    {
        ...
        for (vector<ReputationMod>::iterator itr = modifier->mods.begin(); itr != modifier->mods.end(); ++itr)
        {
            ...
            /* rep limit? */
            if (!IS_INSTANCE(GetMapId()) || (IS_INSTANCE(GetMapId()) && this->iInstanceType != DUNGEON_HEROIC))
            {
                if ((*itr).replimit)
                {
