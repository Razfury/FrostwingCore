...
void MapMgr::BanCheatersByInstanceID(int32 InstanceID)
{
	for (PlayerStorageMap::const_iterator itr = m_PlayerStorage.begin(); itr != m_PlayerStorage.end(); ++itr)
	{
		...
		if (p->GetInstanceID() == InstanceID) // Make sure instance id matches.
			p->SetBanned();
		else
			return;
	}
...
void MapMgr::RemoveAuraFromGroup(uint32 spellId, uint32 mapId)
{
    for (PlayerStorageMap::const_iterator itr = m_PlayerStorage.begin(); itr != m_PlayerStorage.end(); ++itr)
    {
        Player* p = itr->second;
        ...
        if (p->GetInstanceID() != GetInstanceID()) //Instance id doesn't match return
            return;
...
void MapMgr::SendAchievementToAllPlayers(int32 AchID, int32 InstanceID)
{
    for (PlayerStorageMap::const_iterator itr = m_PlayerStorage.begin(); itr != m_PlayerStorage.end(); ++itr)
    {
        Player* p = itr->second;
		...
		if (InstanceID == p->GetInstanceID()) // Make sure instance id matches.
		p->GetAchievementMgr().GMCompleteAchievement(NULL, AchID);
    }
}

void MapMgr::SendAchievementToAllPlayers(int32 AchID)
{
	for (PlayerStorageMap::const_iterator itr = m_PlayerStorage.begin(); itr != m_PlayerStorage.end(); ++itr)
	{
		Player* p = itr->second;
		...
		if (p == NULL)
			return;
		if (p->GetAchievementMgr().HasCompleted(AchID)) // Player has achievement already, just return.
			return;
