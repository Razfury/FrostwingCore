...
void Spell::SpellEffectSchoolDMG(uint32 i) // dmg school
{
    ...
    else
    {
        ...
        switch (GetProto()->NameHash)
        {
			...
			{
				if (GetUnitTarget() && GetUnitTarget()->HasAuraWithMechanics(MECHANIC_ENSNARED) || GetUnitTarget()->HasAura(31589)) // Snared or Slow
				{
					if (u_caster && u_caster->IsPlayer() && TO_PLAYER(u_caster)->HasAura(29447)) //Torment Of The Weak Rank 1
					{
						dmg += dmg * 0.04; // get dmg and add 4% to it.
					}
					if (u_caster && u_caster->IsPlayer() && TO_PLAYER(u_caster)->HasAura(55339)) //Torment Of The Weak Rank 2
					{
						dmg += dmg * 0.08; // get dmg and add 8% to it.
					}
					if (u_caster && u_caster->IsPlayer() && TO_PLAYER(u_caster)->HasAura(55340)) //Torment Of The Weak Rank 3
					{
						dmg += dmg * 0.12; // get dmg and add 12% to it.
					}
				}
			}
			break;
			...
			break;
			case SPELL_HASH_ARCANE_BARRAGE:
			{
				if (GetUnitTarget() && GetUnitTarget()->HasAuraWithMechanics(MECHANIC_ENSNARED) || GetUnitTarget()->HasAura(31589)) // Snared or Slow
				{
					if (u_caster && u_caster->IsPlayer() && TO_PLAYER(u_caster)->HasAura(29447)) //Torment Of The Weak Rank 1
        ...
        switch (GetProto()->Id)
        {
        case 69075: // Lord Marrowgar (Bone Storm)
            if (u_caster != NULL && unitTarget != NULL)
            {
                float dist = u_caster->GetDistance2dSq(unitTarget);
                if (dist >= 9.0f)
                {
                    dist -= 9.0f;
                }
            ...
            break;
		case 52436: // Scarlet Cannon (death knight area)
			break;
	...
	sSpellHookInterface.OnSpellEffect(this, this->u_caster, m_spellInfo->Id, SPELL_EFFECT_SCHOOL_DAMAGE);
    ...
    else
    {
        ...
        else
        {
            if (u_caster != NULL)
            {
				...
				if (GetProto()->ap_override != 0) // Final Check to see if entry has a DB override
				{
					if (p_caster != NULL && p_caster->getClass() != HUNTER) // if class is not a hunter
						dmg = 0;
						dmg = (p_caster->GetAP() * GetProto()->ap_override);
						if (p_caster != NULL && p_caster->getClass() == HUNTER)
						dmg = 0;
						dmg = (p_caster->GetRAP() * GetProto()->ap_override);
				}
				if (GetProto()->ap_dot_override != 0) // Final Check
				{
					if (p_caster != NULL && p_caster->getClass() != HUNTER) // if class is not a hunter
						dmg = 0;
						dmg = (p_caster->GetAP() * GetProto()->ap_dot_override);
						if (p_caster != NULL && p_caster->getClass() == HUNTER)
						dmg = 0;
						dmg = (p_caster->GetRAP() * GetProto()->ap_dot_override);
				}
...
void Spell::SpellEffectDummy(uint32 i) // Dummy(Scripted events)
{
	switch (m_spellInfo->Id)
	{
	case 100: // Charge rank 1 (Warrior)
	{
		if (!this->p_caster)
			return;
		Player* player = this->p_caster;

		if (player->HasAura(12285)) // Improved Charge +5 rage ( 9 + 5 = 14 )
		{
			player->CastSpell(player, 40602, true);
			player->ModPower(POWER_TYPE_RAGE, 140);
		}
		if (player->HasAura(12697)) // Improved Charge +5 rage ( 9 + 10 = 19 )
		{
			player->CastSpell(player, 40602, true);
			player->ModPower(POWER_TYPE_RAGE, 190);
		}
		if (!player->HasAura(12285) && !player->HasAura(12697)) // 9 rage
		{
			player->CastSpell(player, 40602, true);
			player->ModPower(POWER_TYPE_RAGE, 90);
		}
		return;
	}
	case 6178: // Charge rank 2(Warrior)
	{
		if (!this->p_caster)
			return;
		Player* player = this->p_caster;

		if (player->HasAura(12285)) // Improved Charge +5 rage ( 12 + 5 = 17 )
		{
			player->ModPower(POWER_TYPE_RAGE, 170);
		}
		if (player->HasAura(12697)) // Improved Charge +5 rage ( 12 + 10 = 22 )
		{
			player->ModPower(POWER_TYPE_RAGE, 220);
		}
		if (!player->HasAura(12285) && !player->HasAura(12697)) // 12 rage
		{
			player->ModPower(POWER_TYPE_RAGE, 120);
		}
		return;
	}
	case 11578: // Charge rank 3(Warrior)
	{
		if (!this->p_caster)
			return;
		Player* player = this->p_caster;

		if (player->HasAura(12285)) // Improved Charge +5 rage ( 15 + 5 = 20 )
		{
			player->ModPower(POWER_TYPE_RAGE, 200);
		}
		if (player->HasAura(12697)) // Improved Charge +5 rage ( 15 + 10 = 25 )
		{
			player->ModPower(POWER_TYPE_RAGE, 250);
		}
		if (!player->HasAura(12285) && !player->HasAura(12697)) // 15 rage
		{
			player->ModPower(POWER_TYPE_RAGE, 150);
		}
		return;
	}
	case 46584: // Death Knight: Raise Dead
	{
		//Spell Status:
		// Glyph check - working
		if (!this->p_caster)
		...
		float y = player->GetPositionY() - 1;
		float z = player->GetPositionZ();

		if (player->HasAura(59313) && player->HasAura(52143) == false) // Has glyph of raise dead but not master of ghouls
		{
			player->CastSpellAoF(x, y, z, dbcSpell.LookupEntry(46585), true); // 1 min version
			return;
		}
		if (player->HasAura(59313) && player->HasAura(52143)) // Has glyph of raise dead and master of ghouls
		{
			player->CastSpellAoF(x, y, z, dbcSpell.LookupEntry(52150), true); // infinite pet version.
			return;
		}
		if (player->HasAura(59313) == false && player->HasAura(52143) == false) // No glyph of raise dead or master of ghouls so we check for corpse dust first then corpse
		{
			if (player->GetItemInterface()->GetItemCount(37201) >= 1) // Player has corpse dust in inventory do NOT check the bank inventory only.
			{
				player->GetItemInterface()->RemoveItemAmt(37201, 1); // Remove 1 corpse dust from player.
				player->CastSpellAoF(x, y, z, dbcSpell.LookupEntry(46585), true); // 1 min version
				return;
			}
			else // Player has no corpse dust in inventory check for a humanoid corpse within 30 yds
			{
				uint64 guid = player->GetSelection();
				Creature* pCreature = player->GetMapMgr()->GetCreature(GET_LOWGUID_PART(guid));
				if (pCreature && pCreature->IsDead() && pCreature->GetCreatureInfo()->Type == UNIT_TYPE_HUMANOID) //If the creature exists, is dead and is a humanoid type
				{
					player->CastSpellAoF(pCreature->GetPositionX(), pCreature->GetPositionY(), pCreature->GetPositionZ(), dbcSpell.LookupEntry(46585), true); // 1 min version
					return;
				}
				else
				{
					SendCastResult(player, 0, SPELL_FAILED_CUSTOM_ERROR, SPELL_EXTRA_ERROR_UNK9);
					player->ClearCooldownForSpell(46584);
					return;
				}
			}
		}
		if (player->HasAura(59313) == false && player->HasAura(52143) == true) // No glyph of raise dead but has master of ghouls so we check for corpse dust first then corpse
		{
			if (player->GetItemInterface()->GetItemCount(37201) >= 1) // Player has corpse dust in inventory do NOT check the bank inventory only.
			{
				player->GetItemInterface()->RemoveItemAmt(37201, 1); // Remove 1 corpse dust from player.
				player->CastSpellAoF(x, y, z, dbcSpell.LookupEntry(52150), true); // infinite min version
				return;
			}
			else // Player has no corpse dust in inventory check for a humanoid corpse within 30 yds
			{
				...
				if (pCreature && pCreature->IsDead() && pCreature->GetCreatureInfo()->Type == UNIT_TYPE_HUMANOID) //If the creature exists, is dead and is a humanoid type
				{
					player->CastSpellAoF(pCreature->GetPositionX(), pCreature->GetPositionY(), pCreature->GetPositionZ(), dbcSpell.LookupEntry(52150), true); // infinite version
					return;
				}
				else
				{
	...
	{
		Player* player = this->p_caster;
		if (player->HasQuest(12720))
			player->GetQuestLogForEntry(12720)->SendQuestComplete();
		player->GetQuestLogForEntry(12720)->UpdatePlayerFields();
		return;
	}
		//Priest SpellEffectDummy Handlers
	case 47540: // Penance rank 1
	{
		if (!this->p_caster || !this->p_caster->isAlive() ||
			!this->GetUnitTarget() || !this->GetUnitTarget()->isAlive())
			return;

		Unit* target = this->GetUnitTarget();
		Player* player = this->p_caster;

		if (isAttackable(player, target))   // Do holy damage
		{
			if (target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_ATTACKABLE_2))
			{
				SendCastResult(player, 0, SPELL_FAILED_DONT_REPORT, SPELL_EXTRA_ERROR_NONE);
				player->ClearCooldownForSpell(47540);
				return;
			}
			else
			{
				sEventMgr.AddEvent(target, &Unit::EventCastSpell, TO_UNIT(player), target, dbcSpell.LookupEntry(47666), EVENT_UNK, 100, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
				sEventMgr.AddEvent(target, &Unit::EventCastSpell, TO_UNIT(player), target, dbcSpell.LookupEntry(47666), EVENT_UNK, 1000, 2, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
			}
		}
		else // Heal
		{
			if (target->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_ATTACKABLE_2) || target->GetFaction() == 35)
			{
				SendCastResult(player, 0, SPELL_FAILED_DONT_REPORT, SPELL_EXTRA_ERROR_NONE);
				player->ClearCooldownForSpell(47540);
				return;
			...
			{
				sEventMgr.AddEvent(target, &Unit::EventCastSpell, TO_UNIT(player), target, dbcSpell.LookupEntry(47750), EVENT_UNK, 100, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
				sEventMgr.AddEvent(target, &Unit::EventCastSpell, TO_UNIT(player), target, dbcSpell.LookupEntry(47750), EVENT_UNK, 1000, 2, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
			}
		}
		return;
	}
	case 53005: // Penance rank 2
	{
		if (!this->p_caster || !this->p_caster->isAlive() ||
			!this->GetUnitTarget() || !this->GetUnitTarget()->isAlive())
			return;
		...
		if (isAttackable(player, target))   // Do holy damage
		{
			sEventMgr.AddEvent(target, &Unit::EventCastSpell, TO_UNIT(player), target, dbcSpell.LookupEntry(52998), EVENT_UNK, 700, 3, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
		}
		else // Heal
		{
			sEventMgr.AddEvent(target, &Unit::EventCastSpell, TO_UNIT(player), target, dbcSpell.LookupEntry(52983), EVENT_UNK, 700, 3, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
		}
		return;
	}
	case 53006: // Penance rank 3
	{
		if (!this->p_caster || !this->p_caster->isAlive() ||
			!this->GetUnitTarget() || !this->GetUnitTarget()->isAlive())
			return;
		...
		if (isAttackable(player, target))   // Do holy damage
		{
			sEventMgr.AddEvent(target, &Unit::EventCastSpell, TO_UNIT(player), target, dbcSpell.LookupEntry(52999), EVENT_UNK, 700, 3, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
		}
		else // Heal
		{
			sEventMgr.AddEvent(target, &Unit::EventCastSpell, TO_UNIT(player), target, dbcSpell.LookupEntry(52984), EVENT_UNK, 700, 3, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
		}
		return;
	}
	case 53007: // Penance rank 4
	{
		if (!this->p_caster || !this->p_caster->isAlive() ||
			!this->GetUnitTarget() || !this->GetUnitTarget()->isAlive())
			return;
		...
		if (isAttackable(player, target))   // Do holy damage
		{
			sEventMgr.AddEvent(target, &Unit::EventCastSpell, TO_UNIT(player), target, dbcSpell.LookupEntry(53000), EVENT_UNK, 700, 3, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
		}
		else // Heal
		{
			sEventMgr.AddEvent(target, &Unit::EventCastSpell, TO_UNIT(player), target, dbcSpell.LookupEntry(52985), EVENT_UNK, 700, 3, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
		}
		return;
	}
	}

	sSpellHookInterface.OnSpellEffect(this, this->u_caster, m_spellInfo->Id, SPELL_EFFECT_DUMMY); // Call Dummy spell effects if we hooked any.
...
void Spell::SpellEffectTeleportUnits(uint32 i)    // Teleport Units
{
	...
	sSpellHookInterface.OnSpellEffect(this, this->u_caster, m_spellInfo->Id, SPELL_EFFECT_TELEPORT_UNITS);
...
void Spell::SpellEffectHeal(uint32 i) // Heal
{
    ...
    else
    {
		...
		case 20167:
			...
			{
				Player * plr = TO_PLAYER(unitTarget);
				int32 new_dmg = (plr->GetAP() * 0.15) + (plr->GetPosDamageDoneMod(SCHOOL_HOLY) * 0.15);
				Heal(new_dmg);
			}
			break;
		case 50475:
		{
			if (unitTarget)
			{
				uint32 new_dmg = CalculateDamage(u_caster, unitTarget, MELEE, 0, dbcSpell.LookupEntry(12721)) * 0.04; // 4% of damage dealt
				SpellEntry* spellInfo = dbcSpell.LookupEntry(50475);
...
void Spell::SpellEffectSummon(uint32 i)
{
	...
	if (m_spellInfo->Id == 52363) // Call Salanar (give quest credit)
	{
		Player* caster1 = m_caster->GetMapMgr()->GetInterface()->GetPlayerNearestCoords(m_caster->GetPositionX(), m_caster->GetPositionY(), m_caster->GetPositionZ());
			QuestLogEntry* pQuest = caster1->GetQuestLogForEntry(12687);
			if (caster1)
			{
				if (pQuest != NULL)
				{
					pQuest->Finish();
					sQuestMgr.BuildQuestComplete(caster1, QuestStorage.LookupEntry(12687));
					caster1->AddToFinishedQuests(12687);
					pQuest->UpdatePlayerFields();
					caster1->RemoveAura(52275);
					caster1->RemoveAura(52693);
				}
			}
	}
...
void Spell::SpellEffectEnergize(uint32 i) // Energize
{
	...
	case 51206:
		...
		{
			modEnergy = 0;
			if (p_caster->HasAura(49223)) // Rank 1
			{
				p_caster->ModPower(POWER_TYPE_RUNIC_POWER, 30); // Add 2.5 runic power rounded up
			}
			if (p_caster->HasAura(49599)) // Rank 2
			{
				p_caster->ModPower(POWER_TYPE_RUNIC_POWER, 50); // Add 5 runic power rounded up
			}
		}
		break;
...
void Spell::SpellEffectSummonObject(uint32 i)
{
	...
	if (m_spellInfo->Id == 52482) // Death knight Quest: Gift From The Harvester (needs fixed properly)
	{
		if (p_caster)
		{
			p_caster->AddQuestKill(12698, 0);
		}
	}

	if (m_spellInfo->Id == 53061) // Death knight Quest: Ambush At The OVerlook
	{
		if (p_caster)
		{
			p_caster->GetMapMgr()->GetInterface()->SpawnCreature(29076, p_caster->GetPositionX() + RandomUInt(1, 3), p_caster->GetPositionY(), p_caster->GetPositionZ(), p_caster->GetOrientation(), true, false, 0, 0, 4);
		}
	}
...
void Spell::SpellEffectSendEvent(uint32 i) //Send Event
{
	...
	sSpellHookInterface.OnSpellEffect(this, this->u_caster, m_spellInfo->Id, SPELL_EFFECT_SEND_EVENT);
...
void Spell::SpellEffectScriptEffect(uint32 i) // Script Effect
{
	...
	sSpellHookInterface.OnSpellEffect(this, this->u_caster, m_spellInfo->Id, SPELL_EFFECT_SCRIPT_EFFECT);
	...
	if (m_spellInfo->Id == 20271 || m_spellInfo->Id == 53408 || m_spellInfo->Id == 53407) // Temp until new system is handled
	{
...
void Spell::SpellEffectCharge(uint32 i)
{
    if (unitTarget == NULL || !unitTarget->isAlive())
        return;
	if (u_caster->IsStunned() || u_caster->m_rooted || u_caster->m_rooted_moonscript || u_caster->IsPacified() || u_caster->IsFeared())
        return;
...
void Spell::SpellEffectActivateRunes(uint32 i)
{
	...
	{
		if (p_caster->GetRune(j) == RUNE_TYPE_RECHARGING && p_caster->GetRune(j) == GetProto()->EffectMiscValue[i])
		{
