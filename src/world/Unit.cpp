...
void Unit::Update(uint32 p_time)
{
    ...
    if (!IsDead())
    {
		if (HasAura(50510) && HasAura(51726)) // Has both ebon plague and crypt fever
		{
			this->RemoveAura(50510);
		}

		if (HasAura(50510) && HasAura(51734)) // Has both ebon plague and crypt fever
		{
			this->RemoveAura(50510);
		}

		if (HasAura(50510) && HasAura(51735)) // Has both ebon plague and crypt fever
		{
			this->RemoveAura(50510);
		}
        //////////////////////////////////////////////////////////////////////////////////////////
...
uint32 Unit::GetSpellDidHitResult(Unit* pVictim, uint32 weapon_damage_type, SpellEntry* ability)
{
    ...
    if (this->IsPlayer())
    {
		...
		else if (ability && ability->NameHash == SPELL_HASH_AVENGER_S_SHIELD)
		{
			self_skill = TO_PLAYER(this)->_GetSkillLineCurrent(SKILL_DEFENSE);
		}
        if (it)
            ...
            else
            {
                ...
                switch (r)
                {
                    ...
                    case 5:
                    {
						...
						if (IsPlayer())
						{
							sSpellHookInterface.OnCritAttack(TO_PLAYER(this));
							if (ability)
							sSpellHookInterface.OnCriticalAttack(TO_PLAYER(this), ability->Id);
						}
...
void Unit::InstanceCreatureAwardEmblem(Unit* u) // Checks are made to see if the dungeon is heroic or not.
{
	for (PlayerStorageMap::iterator itr = u->GetMapMgr()->m_PlayerStorage.begin(); itr != u->GetMapMgr()->m_PlayerStorage.end(); ++itr)
	{
		(*itr).second->GetItemInterface()->AddItemById(47241, 1, 0); // Emblem of Triumph
	};
};

void Unit::InstanceCreatureAwardAchievement(Unit* u, uint32 achid)
{
	for (PlayerStorageMap::iterator itr = u->GetMapMgr()->m_PlayerStorage.begin(); itr != u->GetMapMgr()->m_PlayerStorage.end(); ++itr)
	{
		if ((*itr).second->GetAchievementMgr().HasCompleted(achid) == false)
			continue;

		(*itr).second->GetAchievementMgr().GMCompleteAchievement(NULL, achid);
	};
};

Player* Unit::GetPlayersInInstance(Unit* u) // Checks are made to see if the dungeon is heroic or not.
{
	for (PlayerStorageMap::iterator itr = u->GetMapMgr()->m_PlayerStorage.begin(); itr != u->GetMapMgr()->m_PlayerStorage.end(); ++itr)
	{
		if (!u->GetMapMgr()->m_PlayerStorage.empty()) // Has 1 or more players so continue
		{
			return (*itr).second;
		}
	};
...
bool Unit::RemoveAura(Aura* aur)
{
	...
	if (aur->GetSpellId() == 53489 || aur->GetSpellId() == 59578) // Do not remove Art Of War
		return false;
...
{
	if (spellId == 53489 || spellId == 59578)
		return false;
...
bool Unit::RemoveAuras(uint32* SpellIds)
{
	...
	if (*SpellIds == 59578 || *SpellIds == 53489)
		return false;
...
{
	if (namehash == SPELL_HASH_THE_ART_OF_WAR)
		return false;
...
void Unit::addImmunityType(uint32 spellId, uint32 immunityType, bool apply)
{
	if (apply)
	{
		if (spellId > 0) // If spell id is higher than 0 then immune the spell.
		{
			this->m_spellIdImmune = spellId;
		}
		if (immunityType == MECHANIC_NONE)
		{
			this->m_immuneSlots[0] = 0;
		}
		if (immunityType == MECHANIC_CHARMED)
		{
			this->m_immuneSlots[1] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_DISORIENTED)
		{
			this->m_immuneSlots[2] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_DISARMED)
		{
			this->m_immuneSlots[3] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_DISTRACED)
		{
			this->m_immuneSlots[4] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_FLEEING)
		{
			this->m_immuneSlots[5] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_CLUMSY)
		{
			this->m_immuneSlots[6] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_ROOTED)
		{
			this->m_immuneSlots[7] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_PACIFIED)
		{
			this->m_immuneSlots[8] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_SILENCED)
		{
			this->m_immuneSlots[9] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_ASLEEP)
		{
			this->m_immuneSlots[10] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_ENSNARED)
		{
			this->m_immuneSlots[11] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_STUNNED)
		{
			this->m_immuneSlots[12] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_FROZEN)
		{
			this->m_immuneSlots[13] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_INCAPACIPATED)
		{
			this->m_immuneSlots[14] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_BLEEDING)
		{
			this->m_immuneSlots[15] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_HEALING)
		{
			this->m_immuneSlots[16] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_POLYMORPHED)
		{
			this->m_immuneSlots[17] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_BANISHED)
		{
			this->m_immuneSlots[18] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_SHIELDED)
		{
			this->m_immuneSlots[19] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_SHACKLED)
		{
			this->m_immuneSlots[20] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_MOUNTED)
		{
			this->m_immuneSlots[21] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_SEDUCED)
		{
			this->m_immuneSlots[22] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_TURNED)
		{
			this->m_immuneSlots[23] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_HORRIFIED)
		{
			this->m_immuneSlots[24] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_INVULNARABLE)
		{
			this->m_immuneSlots[25] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_INTERRUPTED)
		{
			this->m_immuneSlots[26] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_DAZED)
		{
			this->m_immuneSlots[27] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_DISCOVERY)
		{
			this->m_immuneSlots[28] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_INVULNERABLE)
		{
			this->m_immuneSlots[29] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_SAPPED)
		{
			this->m_immuneSlots[30] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
		if (immunityType == MECHANIC_ENRAGED)
		{
			this->m_immuneSlots[31] = 1; // 1 MEANS IMMUNITY IS ADDED
		}
	}
}
...
void Unit::EventCastSpell(Unit* Caster2, Unit* Target, SpellEntry* Sp)
{
	ARCEMU_ASSERT(Sp != NULL);
	Spell* pSpell = sSpellFactoryMgr.NewSpell(Caster2, Sp, true, NULL);
	SpellCastTargets targets(Target->GetGUID());
	pSpell->prepare(&targets);
}
...
void Unit::CheckForCombatBug()
{
	//if (CombatStatus.IsInCombat() && attacking)
}
...
void Unit::SendHopOffVehicle(Unit* vehicleowner, float posX, float posY, float posZ)
{
	WorldPacket data(SMSG_MONSTER_MOVE, 1 + 12 + 4 + 1 + 4 + 4 + 4 + 12 + 8);
	data << GetNewGUID();
...
void Unit::SpawnThenEnterVehicle(uint64 entry, uint32 delay)
{
	...
	if ((GetCurrentVehicle() != NULL) && ( !IsCreature() || IsVehicle()))
		return;
	...
	Creature* p = NULL;
	if (IsCreature())
		p = static_cast< Creature* >(this);

	if ((cp->vehicleid == 0) && (p == NULL))
		return;

	LocationVector v(GetPosition());

	Creature *c = GetMapMgr()->CreateCreature(cp->Id);
	c->Load(cp, v.x, v.y, v.z, v.o);
	c->RemoveFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_SPELLCLICK);
	c->PushToWorld(GetMapMgr());

	// Need to delay this a bit since first the client needs to see the vehicle
	EnterVehicle(c->GetGUID(), delay);
}
