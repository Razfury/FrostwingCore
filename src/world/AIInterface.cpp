    ...
    if (m_isNeutralGuard)
    {
        ...
        for (std::set< Object*>::iterator itrPlr = m_Unit->GetInRangePlayerSetBegin(); itrPlr != m_Unit->GetInRangePlayerSetEnd(); ++itrPlr)
        {
            ...
            if (tmpPlr->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_FEIGN_DEATH))
                continue;
			if (tmpPlr->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_COMBAT_WITHPLAYER))
                continue;
...
bool AIInterface::FindFriends(float dist)
{
    ...
    for (itr = m_Unit->GetInRangeSetBegin(); itr != m_Unit->GetInRangeSetEnd(); itr++)
    {
		...
		if (pUnit->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_DISABLE_COMBAT_WITHPLAYER))
        {
            continue;
        }
	...
	if (sWorld.Mmaps == true)
	{
		if (!Flying())
		{
			if (!CreatePath(x, y, z))
			{
				...
				//HandleEvent(EVENT_LEAVECOMBAT, m_Unit, 7);
				//m_AIState = STATE_EVADE;
				//MoveEvadeReturn();
				return false;
			}
...
void AIInterface::EventUnitDied(Unit* pUnit, uint32 misc1)
{
    ...
    {
		if (m_Unit->IsCreature()) // Heroic emblem of triumph give.
		{
			if (TO< Creature* >(m_Unit)->getLevel() >= 80/*TO< Creature* >(m_Unit)->GetMapMgr()->iInstanceMode == DUNGEON_HEROIC*/ &&
				// Utgarde Keep
				TO< Creature* >(m_Unit)->GetEntry() == 23953 || // Prince Keleseth
				TO< Creature* >(m_Unit)->GetEntry() == 27390 || // Skarvald the Constructor
				TO< Creature* >(m_Unit)->GetEntry() == 27389 || // Dalronn the Controller
				TO< Creature* >(m_Unit)->GetEntry() == 23954 || // Ingvar the Plunderer
				// The Nexus
				TO< Creature* >(m_Unit)->GetEntry() == 26731 ||// Grand Magus Telestra
				TO< Creature* >(m_Unit)->GetEntry() == 26763 ||// Anomalus
				TO< Creature* >(m_Unit)->GetEntry() == 26794 || // Ormorok the Tree-Shaper
				TO< Creature* >(m_Unit)->GetEntry() == 26723 || // Keristrasza
				TO< Creature* >(m_Unit)->GetEntry() == 26796 || // Commander Stoutbeard
				TO< Creature* >(m_Unit)->GetEntry() == 26798 || // Commander Kolurg
				// Azjol-Nerub
				TO< Creature* >(m_Unit)->GetEntry() == 28684 || // Krik'thir the Gatewatcher
				TO< Creature* >(m_Unit)->GetEntry() == 28921 || // Hadronox
				TO< Creature* >(m_Unit)->GetEntry() == 29120 || // Anub'arak
				// Ahn'kahet: The Old Kingdom
				TO< Creature* >(m_Unit)->GetEntry() == 29309 || // Elder Nadox
				TO< Creature* >(m_Unit)->GetEntry() == 29308 || // Prince Taldaram
				TO< Creature* >(m_Unit)->GetEntry() == 29310 || // Jedoga Shadowseeker
				TO< Creature* >(m_Unit)->GetEntry() == 29311 || // Herald Volazj
				TO< Creature* >(m_Unit)->GetEntry() == 30258 || // Amanitar
				// Drak'Tharon Keep
				TO< Creature* >(m_Unit)->GetEntry() == 26630 || // Trollgore
				TO< Creature* >(m_Unit)->GetEntry() == 26631 || // Novos the Summoner
				TO< Creature* >(m_Unit)->GetEntry() == 27483 || // King Dred
				TO< Creature* >(m_Unit)->GetEntry() == 26632 || // The Prophet Tharon'ja
				// The Violet Hold
				TO< Creature* >(m_Unit)->GetEntry() == 29315 || // Erekem
				TO< Creature* >(m_Unit)->GetEntry() == 29316 || // Moragg
				TO< Creature* >(m_Unit)->GetEntry() == 29313 || // Ichoron
				TO< Creature* >(m_Unit)->GetEntry() == 29266 || // Xevozz
				TO< Creature* >(m_Unit)->GetEntry() == 29312 || // Lavanthor
				TO< Creature* >(m_Unit)->GetEntry() == 29314 || // Zuramat the Obliterator
				TO< Creature* >(m_Unit)->GetEntry() == 31134 || // Cyanigosa
				// Gundrak
				TO< Creature* >(m_Unit)->GetEntry() == 29304 || // Slad'ran
				TO< Creature* >(m_Unit)->GetEntry() == 29307 || // Drakkari Colossus
				TO< Creature* >(m_Unit)->GetEntry() == 29573 || // Drakkari Elemental
				TO< Creature* >(m_Unit)->GetEntry() == 29305 || // Moorabi
				TO< Creature* >(m_Unit)->GetEntry() == 29306 || // Gal'darah
				TO< Creature* >(m_Unit)->GetEntry() == 29932 || // Eck the Ferocious
				// Halls of Stone
				TO< Creature* >(m_Unit)->GetEntry() == 27975 || // Maiden of Grief
				TO< Creature* >(m_Unit)->GetEntry() == 27977 || // Krystallus
				TO< Creature* >(m_Unit)->GetEntry() == 27978 || // Sjonnir The Ironshaper
				// The Culling of Stratholme
				TO< Creature* >(m_Unit)->GetEntry() == 26529 || // Meathook
				TO< Creature* >(m_Unit)->GetEntry() == 26530 || // Salramm the Fleshcrafter
				TO< Creature* >(m_Unit)->GetEntry() == 26532 || // Chrono-Lord Epoch
				TO< Creature* >(m_Unit)->GetEntry() == 26533 || // Mal'Ganis
				TO< Creature* >(m_Unit)->GetEntry() == 32273 || // Infinite Corruptor
				// Halls of Lightning
				TO< Creature* >(m_Unit)->GetEntry() == 28586 || // General Bjarngrim
				TO< Creature* >(m_Unit)->GetEntry() == 28587 || // Volkhan
				TO< Creature* >(m_Unit)->GetEntry() == 28546 || // Ionar
				TO< Creature* >(m_Unit)->GetEntry() == 28923 || // Loken
				// The Oculus
				TO< Creature* >(m_Unit)->GetEntry() == 27654 || // Drakos the Interrogator
				TO< Creature* >(m_Unit)->GetEntry() == 27447 || // Varos Cloudstrider
				TO< Creature* >(m_Unit)->GetEntry() == 27655 || // Mage-Lord Urom
				TO< Creature* >(m_Unit)->GetEntry() == 27656 || // Ley-Guardian Eregos
				// Utgarde Pinnacle
				TO< Creature* >(m_Unit)->GetEntry() == 26668 || // Svala Sorrowgrave
				TO< Creature* >(m_Unit)->GetEntry() == 26687 || // Gortok Palehoof
				TO< Creature* >(m_Unit)->GetEntry() == 26693 || // Skadi the Ruthless
				TO< Creature* >(m_Unit)->GetEntry() == 26861) // King Ymrion)
			{
				TO< Creature* >(m_Unit)->InstanceCreatureAwardEmblem(TO< Creature* >(m_Unit));
			}
		}
