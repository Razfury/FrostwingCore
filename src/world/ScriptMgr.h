...
enum SpellHookEvents
{
	SPELL_EVENT_KILL_PLAYER = 1,
	SPELL_EVENT_DEATH = 2,
	SPELL_EVENT_CAST_SPELL = 3,
	SPELL_EVENT_ON_TICK = 4,
	SPELL_EVENT_AURA_REMOVE = 5,
	SPELL_EVENT_RESURRECT = 6,
	SPELL_EVENT_EFFECT = 7,
	SPELL_EVENT_AURA = 8,
	SPELL_EVENT_CALCULATE_EFFECT = 9,
	SPELL_EVENT_AFTER_CAST_SPELL = 10,
	SPELL_EVENT_ON_CRIT_ATTACK = 11,
	SPELL_EVENT_ON_CRITICAL_ATTACK = 12,
...
/* SpellHook typedefs */
typedef void(*shOnKillPlayer)(Player* pPlayer, Player* pVictim);
typedef void(*shOnDeath)(Player* pPlayer);
typedef bool(*shOnCastSpell)(Player* pPlayer, SpellEntry* pSpell, Spell* spell);
typedef bool(*shOnAfterCastSpell)(Player* pPlayer, SpellEntry* pSpell, Spell* spell);
typedef bool(*shOnCritAttack)(Player* pPlayer);
typedef bool(*shOnCriticalAttack)(Player* pPlayer, uint32 spellId);
typedef void(*shOnTick)();
typedef void(*shOnAuraRemove)(Aura* aura);
typedef bool(*shOnResurrect)(Player* pPlayer);
typedef void(*shOnSpellEffect)(Spell* sp, Unit* spellcaster, uint32 spellId, uint32 SpellEffectType);
typedef void(*shOnSpellAura)(Aura* aur, uint32 spellId, bool apply, uint32 SpellAuraType);
typedef int32(*shOnCalculateEffect)(Spell* pSpell, uint32 spellId, uint32 i, Unit* target, int32 value);
...
typedef bool(*exp_handle_effect_spell)(uint32 i, Spell* pSpell);
...
typedef HM_NAMESPACE::hash_map< uint32, exp_handle_effect_spell > HandleEffectSpellMap;
...
typedef set<void*> SpellHookList;
...
class SERVER_DECL ScriptMgr : public Singleton<ScriptMgr>
{
	...
	friend class SpellHookInterface;
	...
	void RegisterSpellType(SpellHookEvents event, void* function_pointer);
	...
	//************************************
	// Purpose :    Returns true if ScriptMgr has registered the specified function ptr to the specified event.
	// Parameter:    SpellHookEvents - the event number
	// Parameter:    void * - the function pointer to search for.
	//************************************
	bool has_spell_hook(SpellHookEvents, void*) const;
	...
	SpellHookList _spellhooks[NUM_SPELL_HOOKS];
...
class SERVER_DECL SpellHookInterface : public Singleton<SpellHookInterface>
{
public:
	friend class ScriptMgr;

	void OnKillPlayer(Player* pPlayer, Player* pVictim); // Handled in player.cpp
	void OnDeath(Player* pPlayer); // Handled in player.cpp
	bool OnCastSpell(Player* pPlayer, SpellEntry* pSpell, Spell* spell); // Handled in spell.cpp
	bool OnAfterCastSpell(Player* pPlayer, SpellEntry* pSpell, Spell* spell); // Handled in spell.cpp
	void OnCritAttack(Player* pPlayer); // Handled in unit.cpp
	void OnCriticalAttack(Player* pPlayer, uint32 spellId); // Handled in unit.cpp
	void OnAuraRemove(Aura* aura); // Handled in SpellAuras.cpp
	bool OnResurrect(Player* pPlayer);
	// Spell_Effect Hooks //
	void OnSpellEffect(Spell* sp, Unit* spellCaster, uint32 spellId, uint32 SpellEffectType);
	void OnSpellAura(Aura* aur, uint32 spellId, bool apply, uint32 SpellAuraType);
	int32 OnCalculateEffect(Spell* pSpell, uint32 spellId, uint32 i, Unit* target, int32 value);
};

#define sScriptMgr ScriptMgr::getSingleton()
#define sHookInterface HookInterface::getSingleton()
#define sSpellHookInterface SpellHookInterface::getSingleton()
