...
// SpellDbc.dbc Information:
// Effect[0] = Effect_1 in DBC SpellDbc
// Effect[1] = Effect_2 in DBC SpellDbc
// Effect[2] = Effect_3 in DBC SpellDbc
...
void ApplyNormalFixes()
{
    ...
    for (uint32 x = 0; x < cnt; x++)
    {
        ...
        */
        ...
        sp->proc_interval = 0;//trigger at each event sp->
        sp->c_is_flags = 0;
        ...
        if (strstr(sp->Name, "Lightning Shield") && sp->EffectTriggerSpell[0])
        {
            ...
            sp->proc_interval = RandomUInt(3000, 6000); //few seconds
        }
    ...
    for (uint32 x = 0; x < cnt; x++)
    {
		...
		// Effect[0] = Effect_1 in DBC
		// Effect[1] = Effect_2 in DBC
		// Effect[2] = Effect_3 in DBC
	...
	//Paladin - Judgement
	sp = CheckAndReturnSpellEntry(20271);
	if (sp != NULL)
		sp->Effect[0] = SPELL_EFFECT_SCRIPT_EFFECT;
		sp->CasterAuraState = 0; // Cronic: Do not remove fixes Judgement spell for now until we get caster states working :{

	sp = CheckAndReturnSpellEntry(53408);
	if (sp != NULL)
		sp->Effect[0] = SPELL_EFFECT_SCRIPT_EFFECT;
		sp->CasterAuraState = 0; // Cronic: Do not remove fixes Judgement spell for now until we get caster states working :{

	sp = CheckAndReturnSpellEntry(53407);
	if (sp != NULL)
		sp->Effect[0] = SPELL_EFFECT_SCRIPT_EFFECT;
		sp->CasterAuraState = 0; // Cronic: Do not remove fixes Judgement spell for now until we get caster states working :{
    ...
    if (sp != NULL)
        sp->proc_interval = RandomUInt(3000, 8000);
	...
	// Righteousness always hits regardless.
	sp = CheckAndReturnSpellEntry(20187); // Should it be 21084?
	if (sp != NULL)
		sp->always_apply = true;
	...
	//Paladin - Avenger's Shield
	sp = CheckAndReturnSpellEntry(31935);
	if (sp != NULL)
	{
		sp->Effect[0] = SPELL_EFFECT_SCHOOL_DAMAGE;
	}
	sp = CheckAndReturnSpellEntry(32699);
	if (sp != NULL)
	{
		sp->Effect[0] = SPELL_EFFECT_SCHOOL_DAMAGE;
	}
	sp = CheckAndReturnSpellEntry(32700);
	if (sp != NULL)
	{
		sp->Effect[0] = SPELL_EFFECT_SCHOOL_DAMAGE;
	}
	sp = CheckAndReturnSpellEntry(48826);
	if (sp != NULL)
	{
		sp->Effect[0] = SPELL_EFFECT_SCHOOL_DAMAGE;
	}
	sp = CheckAndReturnSpellEntry(48827);
	if (sp != NULL)
	{
		sp->Effect[0] = SPELL_EFFECT_SCHOOL_DAMAGE;
	}
	...
	//Submerge - Anub'arak
	//Info: override default blizzard effects because we don't support transform and it causes boss to go OOC.
	sp = CheckAndReturnSpellEntry(53421);
	if (sp != NULL)
	{
		sp->Effect[0] = SPELL_AURA_DUMMY;
		sp->Effect[1] = SPELL_AURA_DUMMY;
	}
	sp = CheckAndReturnSpellEntry(52440);
	if (sp != NULL)
	{
		sp->Effect[0] = SPELL_AURA_DUMMY;
		sp->Effect[1] = SPELL_AURA_DUMMY;
	...
	/*sp = CheckAndReturnSpellEntry(53302);
	if( sp != NULL )
		sp->EffectTriggerSpell[0] = 64418
	sp = CheckAndReturnSpellEntry(53303);
	if( sp != NULL )
		sp->EffectTriggerSpell[0] = 64419;
	sp = CheckAndReturnSpellEntry(53304);
	if( sp != NULL )
		sp->EffectTriggerSpell[0] = 64420;
	sp = CheckAndReturnSpellEntry(64418);*/
	if( sp != NULL )
		sp->AuraInterruptFlags |= AURA_INTERRUPT_ON_MOVEMENT;
	sp = CheckAndReturnSpellEntry(64419);
	if( sp != NULL )
		sp->AuraInterruptFlags |= AURA_INTERRUPT_ON_MOVEMENT;
	sp = CheckAndReturnSpellEntry(64420);
	if( sp != NULL )
		sp->AuraInterruptFlags |= AURA_INTERRUPT_ON_MOVEMENT;
    ...
    // Insert rogue spell fixes here
	// Hunger for Blood
	/*sp = CheckAndReturnSpellEntry(51662);
	if (sp != NULL)
	{
		sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
		sp->EffectTriggerSpell[0] = 60177;
		sp->maxstack = 3;
	}*/
    ...
    // Penance
    sp = CheckAndReturnSpellEntry(47540); // rank 1
    if (sp != NULL)
    {
		sp->AttributesEx = 0;
    ...
    sp = CheckAndReturnSpellEntry(53005); // rank 2
    if (sp != NULL)
    {
		sp->AttributesEx = 0;
    ...
    sp = CheckAndReturnSpellEntry(53006); // rank 3
    if (sp != NULL)
    {
		sp->AttributesEx = 0;
        sp->DurationIndex = 566;
    ...
    sp = CheckAndReturnSpellEntry(53007); // rank 4
    if (sp != NULL)
    {
		sp->AttributesEx = 0;
        sp->DurationIndex = 566;
    ...
    // Insert shaman spell fixes here
	// Static Shock
	sp = CheckAndReturnSpellEntry(51525);
	if (sp != NULL)
	{
		sp->procFlags = PROC_ON_MELEE_ATTACK;
		sp->procChance = 2;
	}
	sp = CheckAndReturnSpellEntry(51526);
	if (sp != NULL)
	{
		sp->procFlags = PROC_ON_MELEE_ATTACK;
		sp->procChance = 4;
	}
	sp = CheckAndReturnSpellEntry(51527);
	if (sp != NULL)
	{
		sp->procFlags = PROC_ON_MELEE_ATTACK;
		sp->procChance = 6;
	...
	sp = dbcSpell.LookupEntryForced(33757); // Windfury Enchantmanet
	if( sp != NULL )
	{
		sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
		sp->procFlags = PROC_ON_MELEE_ATTACK;
		sp->EffectTriggerSpell[0] = 33750; //offhand
		sp->procChance = 20;
		sp->proc_interval = 8000;
		sp->maxstack = 1;
	}
	sp = dbcSpell.LookupEntryForced(63373); //Frozen Power
	if( sp != NULL )
	{
	...
	//if( sp != NULL )
	{
		...
		sp->procCharges = 1;
		sp->procFlags = PROC_ON_CAST_SPELL;
		sp->c_is_flags |= SPELL_FLAG2_IS_ALLOWED_TO_LEAVE_CHAINED_AURAS;
	}
    ...
    **********************************************************/
	...
	//Warlock - Decimation 20%
	sp = dbcSpell.LookupEntryForced(63165); // Rank 1
	if (sp != NULL)
	{
		sp->EffectApplyAuraName[0] = SPELL_AURA_ADD_PCT_MODIFIER;
		sp->EffectMiscValue[0] = SMT_CAST_TIME;
		sp->Effect[1] = SPELL_EFFECT_APPLY_AURA;
		sp->EffectApplyAuraName[1] = SPELL_AURA_REMOVE_REAGANT_COST; // Remove Reagant Cost

	}

	sp = dbcSpell.LookupEntryForced(63167); // Rank 2
	if (sp != NULL)
	{
		sp->EffectApplyAuraName[0] = SPELL_AURA_ADD_PCT_MODIFIER;
		sp->EffectMiscValue[0] = SMT_CAST_TIME;
	...
	if( sp != NULL )
	{
		...
		sp->procFlags |= PROC_ON_SPELL_HIT_VICTIM | static_cast<uint32>(PROC_TARGET_SELF);
    ...
    **************************************************************/
	...
	//Ball of Flames - Svala Sorrowgrave
	sp = CheckAndReturnSpellEntry(48246);
	if (sp != NULL)
	{
		sp->MaxTargets = 1;
	}
	...
	//Awaken Subboss
	sp = CheckAndReturnSpellEntry(47669);
	if (sp != NULL)
	{
		sp->MaxTargets = 2; // Caster + Target
	}
    ...
    */
	...
	sp = dbcSpell.LookupEntryForced(53341);
	if (sp != NULL)
	{
		sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
		sp->procFlags = PROC_ON_MELEE_ATTACK;
		sp->EffectTriggerSpell[0] = 53386; //offhand
		sp->procChance = 20;
		sp->proc_interval = 8000;
		sp->maxstack = 2;
	}
	...
	// Dirge Rank 1
	sp = dbcSpell.LookupEntryForced(49223);
	if (sp != NULL)
	{
		sp->procFlags = PROC_ON_CAST_SPELL;
		sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
		...
		sp->ProcOnNameHash[2] = SPELL_HASH_PLAGUE_STRIKE;
		//sp->procChance = 99;
	}
	// Dirge Rank 2
	sp = dbcSpell.LookupEntryForced(49599);
	if (sp != NULL)
	{
		sp->procFlags = PROC_ON_CAST_SPELL;
		sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
	...
	if (sp != NULL)
	{
		...
		sp->ProcOnNameHash[0] = SPELL_HASH_SCOURGE_STRIKE;
		sp->ProcOnNameHash[1] = SPELL_HASH_PLAGUE_STRIKE;
	}
	// Desecration Rank 2
	...
	if (sp != NULL)
	{
		sp->procFlags = PROC_ON_CAST_SPELL;
		sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
		sp->EffectTriggerSpell[0] = 68766;
		sp->ProcOnNameHash[0] = SPELL_HASH_SCOURGE_STRIKE;
		sp->ProcOnNameHash[1] = SPELL_HASH_PLAGUE_STRIKE;
	}
	// Rime
	sp = CheckAndReturnSpellEntry(49188);   //Rank 1
	if (sp != NULL)
	{
		sp->procFlags = PROC_ON_CAST_SPELL;
		sp->ProcOnNameHash[0] = SPELL_HASH_OBLITERATE;
	}
	sp = CheckAndReturnSpellEntry(56822);   //Rank 2
	if (sp != NULL)
	{
		sp->procFlags = PROC_ON_CAST_SPELL;
		sp->ProcOnNameHash[0] = SPELL_HASH_OBLITERATE;
	sp = CheckAndReturnSpellEntry(59057);   //Rank 3
		if (sp != NULL)
		{
			sp->procFlags = PROC_ON_CAST_SPELL;
			sp->ProcOnNameHash[0] = SPELL_HASH_OBLITERATE;
    ...
    // Killing Machine
	sp = CheckAndReturnSpellEntry(51124);
	if (sp != NULL)
	{
		sp->AuraInterruptFlags = AURA_INTERRUPT_ON_CAST_SPELL;
		//sp->AuraInterruptFlags = AURA_INTERRUPT_ON_AFTER_CAST_SPELL;
	}
	sp = CheckAndReturnSpellEntry(51123);
	if (sp != NULL)
	{
		//sp->Effect[0] = SPELL_EFFECT_NULL;
		sp->EffectTriggerSpell[0] = 51124;
		sp->procFlags = PROC_ON_MELEE_ATTACK;
		sp->procChance = 4;
	}
	sp = CheckAndReturnSpellEntry(51127);
	if (sp != NULL)
	{
		//sp->Effect[0] = SPELL_EFFECT_NULL;
		sp->EffectTriggerSpell[0] = 51124;
	...
	sp = CheckAndReturnSpellEntry(51128);
	if (sp != NULL)
	{
		//sp->Effect[0] = SPELL_EFFECT_NULL;
		sp->EffectTriggerSpell[0] = 51124;
	...
	sp = CheckAndReturnSpellEntry(51129);
	if (sp != NULL)
	{
		//sp->Effect[0] = SPELL_EFFECT_NULL;
		sp->EffectTriggerSpell[0] = 51124;
	...
	sp = CheckAndReturnSpellEntry(51130);
	if (sp != NULL)
	{
		//sp->Effect[0] = SPELL_EFFECT_NULL;
		sp->EffectTriggerSpell[0] = 51124;
    ...
    **********************************************************/
    ...
    if (sp != NULL)
    {
        ...
        sp->EffectMiscValue[2] = RUNE_TYPE_UNHOLY;
    }
	...
	//Righteous Vengeance (Thanks @Sandshroud)
    ...
    **********************************************************/
	...
	//Herkuml War Totem
	sp = CheckAndReturnSpellEntry(71397);
	if (sp != NULL)
	{
		sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
		sp->procFlags = PROC_ON_MELEE_ATTACK | PROC_ON_RANGED_ATTACK;
	...
	//Deathbringer's Will (Normal Mode)
	sp = CheckAndReturnSpellEntry(71519);
	if (sp != NULL)
	{
		sp->Effect[0] = 6;
		sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
		sp->procFlags = PROC_ON_MELEE_ATTACK | PROC_ON_RANGED_ATTACK;
		sp->procChance = 16.6;
		sp->EffectTriggerSpell[0] = 71484;
		sp->maxstack = 1;
		sp->Effect[1] = 6;
		sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
		sp->procFlags = PROC_ON_MELEE_ATTACK | PROC_ON_RANGED_ATTACK;
		sp->procChance = 16.6;
		sp->EffectTriggerSpell[1] = 71485;
		sp->maxstack = 1;
		sp->Effect[2] = 6;
		sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
		sp->procFlags = PROC_ON_MELEE_ATTACK | PROC_ON_RANGED_ATTACK;
		sp->procChance = 16.6;
		sp->EffectTriggerSpell[2] = 71486;
		sp->maxstack = 1;
		sp->Effect[3] = 6;
		sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
		sp->procFlags = PROC_ON_MELEE_ATTACK | PROC_ON_RANGED_ATTACK;
		sp->procChance = 16.6;
		sp->EffectTriggerSpell[3] = 71487;
		sp->maxstack = 1;
		sp->Effect[4] = 6;
		sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
		sp->procFlags = PROC_ON_MELEE_ATTACK | PROC_ON_RANGED_ATTACK;
		sp->procChance = 16.6;
		sp->EffectTriggerSpell[4] = 71491;
		sp->maxstack = 1;
		sp->Effect[5] = 6;
		sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
		sp->procFlags = PROC_ON_MELEE_ATTACK | PROC_ON_RANGED_ATTACK;
		sp->procChance = 16.6;
		sp->EffectTriggerSpell[5] = 71492;
		sp->maxstack = 1;
	}

	//Deathbringer's Will (Heroic Mode)
	sp = CheckAndReturnSpellEntry(71562);
	if (sp != NULL)
	{
			sp->Effect[0] = 6;
			sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
			sp->procFlags = PROC_ON_MELEE_ATTACK | PROC_ON_RANGED_ATTACK;
			sp->procChance = 16.6;
			sp->EffectTriggerSpell[0] = 71561;
			sp->proc_interval = 30000; // Make sure can't proc again for 30 seconds..
			sp->Effect[1] = 6;
			sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
			sp->procFlags = PROC_ON_MELEE_ATTACK | PROC_ON_RANGED_ATTACK;
			sp->procChance = 16.6;
			sp->EffectTriggerSpell[1] = 71556;
			sp->proc_interval = 30000; // Make sure can't proc again for 30 seconds..
			sp->Effect[2] = 6;
			sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
			sp->procFlags = PROC_ON_MELEE_ATTACK | PROC_ON_RANGED_ATTACK;
			sp->procChance = 16.6;
			sp->EffectTriggerSpell[2] = 71558;
			sp->proc_interval = 30000; // Make sure can't proc again for 30 seconds..
			sp->Effect[3] = 6;
			sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
			sp->procFlags = PROC_ON_MELEE_ATTACK | PROC_ON_RANGED_ATTACK;
			sp->procChance = 16.6;
			sp->EffectTriggerSpell[3] = 71557;
			sp->proc_interval = 30000; // Make sure can't proc again for 30 seconds..
			sp->Effect[4] = 6;
			sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
			sp->procFlags = PROC_ON_MELEE_ATTACK | PROC_ON_RANGED_ATTACK;
			sp->procChance = 16.6;
			sp->EffectTriggerSpell[4] = 71559;
			sp->proc_interval = 30000; // Make sure can't proc again for 30 seconds..
			sp->Effect[5] = 6;
			sp->EffectApplyAuraName[0] = SPELL_AURA_PROC_TRIGGER_SPELL;
			sp->procFlags = PROC_ON_MELEE_ATTACK | PROC_ON_RANGED_ATTACK;
			sp->procChance = 16.6;
			sp->EffectTriggerSpell[5] = 71560;
			sp->proc_interval = 30000; // Make sure can't proc again for 30 seconds..
	}
	...
	//Blood boil
	sp = CheckAndReturnSpellEntry(48721);
	if (sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_ALL_ENEMY_IN_AREA;
		sp->EffectImplicitTargetB[0] = EFF_TARGET_NONE;
	}
	sp = CheckAndReturnSpellEntry(49939);
	if (sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_ALL_ENEMY_IN_AREA;
		sp->EffectImplicitTargetB[0] = EFF_TARGET_NONE;
	}
	sp = CheckAndReturnSpellEntry(49940);
	if (sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_ALL_ENEMY_IN_AREA;
		sp->EffectImplicitTargetB[0] = EFF_TARGET_NONE;
	}
	sp = CheckAndReturnSpellEntry(49941);
	if (sp != NULL)
	{
		sp->EffectImplicitTargetA[0] = EFF_TARGET_ALL_ENEMY_IN_AREA;
		sp->EffectImplicitTargetB[0] = EFF_TARGET_NONE;
