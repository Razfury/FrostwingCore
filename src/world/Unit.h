...
enum UnitFieldFlags // UNIT_FIELD_FLAGS #46 - these are client flags
{
	...
	UNIT_FLAG_ATTACKABLE_ALL			 = 0x00000000, // 0            0  makes creature attackable and removes all flags.
    ...
    UNIT_FLAG_DISABLE_COMBAT_WITHPLAYER  = 0x00000100, // 9          256  changes attackable status
    UNIT_FLAG_DISABLE_COMBAT_WITHNPC     = 0x00000200, // 10         512  ? some NPCs have this
...
class SERVER_DECL Unit : public Object
{
    public:
		...
		void EventCastSpell(Unit* Caster2, Unit* Target, SpellEntry* Sp);
		...
		//Custom
		void InstanceCreatureAwardEmblem(Unit* u);
		void InstanceCreatureAwardAchievement(Unit* u, uint32 achid);
		Player* GetPlayersInInstance(Unit* u);
		...
		void addImmunityType(uint32 spellId, uint32 immunityType, bool apply);
		bool hasImmunityType(uint32 spellId, uint32 immunityType);
		...
		uint32 m_spellIdImmune;
		uint32 m_immuneSlots[32];
        uint32 m_addDmgOnce;
		...
		Unit* m_attacking;
		void Root_Moonscript();
		void Unroot_Moonscript();
		bool isRooted_Moonscript()
		{

			if (m_rooted_moonscript)
				return true;
			else
				return false;
		}
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public:
		...
		void SpawnThenEnterVehicle(uint64 entry, uint32 delay);
		...
		void SendHopOffVehicle(Unit* vehicleowner, float posX, float posY, float posZ);
