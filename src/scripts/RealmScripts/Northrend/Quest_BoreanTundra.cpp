...
class PurifyingTotemAI : public HybridCreatureScriptAI
{
        HYBRIDSCRIPT_FUNCTION(PurifyingTotemAI, HybridCreatureScriptAI);
        PurifyingTotemAI(Creature* pCreature) : HybridCreatureScriptAI(pCreature)
        {
...
class SeaforiumDepthCharge : public HybridCreatureScriptAI
{
        HYBRIDSCRIPT_FUNCTION(SeaforiumDepthCharge, HybridCreatureScriptAI);
        SeaforiumDepthCharge(Creature* pCreature) : HybridCreatureScriptAI(pCreature)
        {
            SetCanMove(false);
            SetCanEnterCombat(false);
            _unit->SetFaction(21);
...
public:
	void GossipHello(Object* pObject, Player* pPlayer)
	{
		...
		objmgr.CreateGossipMenuForPlayer(&Menu, pObject->GetGUID(), 12435, pPlayer);
		if (sEAS.GetQuest(pPlayer, 11794) && pPlayer->HasAura(46078))
			Menu->AddItem(ICON_CHAT, "I have reason to believe you're involved in cultist activity.", 1);
	...
	void GossipSelectOption(Object* pObject, Player*  pPlayer, uint32 Id, uint32 IntId, const char* Code)
	{
		...
		{
		case 1:
			Creature* SaltyJohn = TO_CREATURE(pObject);
...
public:
	void GossipHello(Object* pObject, Player* pPlayer)
	{
		...
		objmgr.CreateGossipMenuForPlayer(&Menu, pObject->GetGUID(), 12435, pPlayer);
		if (sEAS.GetQuest(pPlayer, 11794) && pPlayer->HasAura(46078)) // Has quest and has used the item.
			Menu->AddItem(ICON_CHAT, "What do you know about the Cult of the Damned?", 1);
	...
	void GossipSelectOption(Object* pObject, Player*  pPlayer, uint32 Id, uint32 IntId, const char* Code)
	{
		...
		{
		case 1:
			Creature* TomHegger = TO_CREATURE(pObject);
...
public:
	void GossipHello(Object* pObject, Player* pPlayer)
	{
		...
		objmgr.CreateGossipMenuForPlayer(&Menu, pObject->GetGUID(), 12435, pPlayer);
		if (sEAS.GetQuest(pPlayer, 11794) && pPlayer->HasAura(46078))
			Menu->AddItem(ICON_CHAT, "How long have you worked for the Cult of the Damned?", 1);
	...
	void GossipSelectOption(Object* pObject, Player*  pPlayer, uint32 Id, uint32 IntId, const char* Code)
	{
		...
		{
		case 1:
			Creature* GuardMitch = TO_CREATURE(pObject);
