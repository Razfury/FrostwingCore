...
class ArtOfWarSpellProc : public SpellProc
{
	SPELL_PROC_FACTORY_FUNCTION(ArtOfWarSpellProc);
	...
	bool DoEffect(Unit* victim, SpellEntry* CastingSpell, uint32 flag, uint32 dmg, uint32 abs, int* dmg_overwrite, uint32 weapon_damage_type)
	{
		// If was a crit allow proc
		if ((flag & PROC_ON_CRIT_ATTACK))
		{
			if (victim->HasAura(53486))
			{
				victim->CastSpell(victim, 53489, true);
			}
			if (victim->HasAura(53488))
			{
				victim->CastSpell(victim, 59578, true);
			}
			return false;
		}
...
class SealOfLightSpellProc : public SpellProc
{
	SPELL_PROC_FACTORY_FUNCTION(SealOfLightSpellProc);

	bool CanProc(Unit* victim, SpellEntry* CastingSpell)
	{
		int32 proc_chance = RandomUInt(0, 100);
		if (proc_chance >= 26)
			return false;
...
void SpellProcMgr::SetupPaladin()
{
	...
	AddById(53486, &ArtOfWarSpellProc::Create);
	AddById(53488, &ArtOfWarSpellProc::Create);
	AddById(20165, &SealOfLightSpellProc::Create);
