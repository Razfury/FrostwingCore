...
bool Group::AddMember(PlayerInfo* info, int32 subgroupid/* =-1 */)
{
	...
	m_GroupMembers_2.insert(info); // add player to set

    if (m_isqueued)
    {
        m_isqueued = false;
        BattlegroundManager.RemoveGroupFromQueues(this);
		sLfgMgr.Leave(pPlayer, NULL);
    }
...
void Group::RemovePlayer(PlayerInfo* info)
{
    ...
    WorldPacket data(50);
    Player* pPlayer = info->m_loggedInPlayer;
	m_GroupMembers_2.erase(info); // Remove player from groupmembers set
	...
	if (isLFGGroup())
	{
		uint64 gguid = this->GetGUID();
		sLfgMgr.Leave(pPlayer, this); // Player Left so let them know.
		sLfgMgr.OfferContinue(this); // Offer leader to requeue in the random dungeon finder.
		if (pPlayer->GetMapMgr()->GetMapInfo()->type == INSTANCE_MULTIMODE) // If player is in a dungeon tele him out.
		{
			sLfgMgr.TeleportPlayer(pPlayer, true); // Boot player from instance regardless.
		}
	}
