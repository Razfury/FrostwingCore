...
#define BANNER "<< FrostwingCore 3.3.5a %s/%s-%s (%s) :: World Server >>"
...
bool Master::Run(int argc, char** argv)
{
    ...
    sLog.outBasic("The key combination <Ctrl-C> will safely shut down the server, failure to do this will cause data loss and players will NOT be saved.");
	...
	Log.Success("Server", "FrostwingCore has been initialized and is now ready for players!              Startup time: %ums", LoadingTime);
...
static const char *REQUIRED_WORLD_DB_VERSION = "WORLD_DB_UPDATE_2";
...
bool Master::CheckDBVersion()
{
    ...
    if (result != 0)
    {
        ...
        if (result < 0)
        {
            ...
            Log.Error("Database", "You can find the world update queries in the FrostwingCoreDB\Database Updates of your FrostwingCore source directory.");
        }
        else
            Log.Error("Database", "Your world database is probably too new for this FrostwingCore version, you need to update your server. Exiting.");
    ...
    if (result != 0)
    {
        ...
        if (result < 0)
        {
            ...
            Log.Error("Database", "You can find the character update queries in the FrostwingCoreDB\Database Updates directory of the FrostwingCore source directory.");
        }
        else
...
{
	Log.Error("Crash Handler", "Server has crashed... initializing crash handler.");
    ...
    {
        Log.Error("sql", "An error occured while trying to save data. Report crash to FrostwingCore dev team.");
    }
