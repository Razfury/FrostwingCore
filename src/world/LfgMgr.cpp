...
void LfgMgr::InitializeLockedDungeons(Player* player)
{
	...
	for (LfgDungeonSet::const_iterator it = dungeons.begin(); it != dungeons.end(); ++it)
	{
		...
		/*else if (dungeon->ID == 255 && player->GetAverageItemLevel() < 180) // Random WOTLK dungeons minimum 180 item level to enter/queue.
		locktype = LFG_LOCKSTATUS_TOO_LOW_GEAR_SCORE;
		else if (dungeon->ID == 256 && player->GetAverageItemLevel() < 180) // Random WOTLK dungeons minimum 180 item level to enter/queue.
		locktype = LFG_LOCKSTATUS_TOO_LOW_GEAR_SCORE;*/
			...
			for (itx = grp->GetSubGroup(0)->GetGroupMembersBegin(); itx != grp->GetSubGroup(0)->GetGroupMembersEnd(); ++itx)
			{
				if (Player* plrg = (*itx)->m_loggedInPlayer)
					if (joinData.result == LFG_JOIN_OK)
					{
						...
						++memberCount;
						players.insert(plrg);
						Log.Debug("LFGMgr", "Group tried queing but something went wrong!", guid);
					}
			}
			if (memberCount != grp->MemberCount())
				//joinData.result = LFG_JOIN_OK;
...
void LfgMgr::RewardDungeonDoneFor(const uint32 dungeonId, Player* player)
{
	...
	Quest* qReward = QuestStorage.LookupEntry(reward->firstQuest);
...

	{
		...
		qReward = QuestStorage.LookupEntry(reward->firstQuest);
