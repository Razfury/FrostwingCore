...
void MoonInstanceScript::sendLFGRewards()
{
	...
	{
		if ((*itr).second->GetGroup()->isLFGGroup())
		{
			if ((*itr).second->GetQuestRewardStatus(24788) == 0) // Not complete
			{
				(*itr).second->GetItemInterface()->AddItemById(49426, 2, 0); // 2 emblems of frost
			}
			else if ((*itr).second->GetQuestRewardStatus(24788) == 1) // complete
			{
				(*itr).second->GetItemInterface()->AddItemById(47241, 2, 0); // 2 emblems of triumph
			}
		}
	};
};
...
void MoonInstanceScript::BuildEncounterMap()
{
    ...
    QueryResult* KillResult = WorldDatabase.Query("SELECT id, entry FROM creature_spawns WHERE map = %u AND entry IN (SELECT entry FROM creature_names WHERE encounter = 1)", mInstance->GetMapId());
    if (KillResult != NULL)
    {
        uint32 Id = 0, Entry = 0;
