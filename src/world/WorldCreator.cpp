...
uint32 InstanceMgr::PreTeleport(uint32 mapid, Player* plr, uint32 instanceid)
{
    ...
    if (pGroup == NULL &&
        (inf->type == INSTANCE_NONRAID && plr->iInstanceType == DUNGEON_HEROIC) &&
        !plr->TriggerpassCheat)
        return INSTANCE_ABORT_NOT_IN_RAID_GROUP;
    ...
    if (inf->type == INSTANCE_NONRAID && plr->GetDungeonDifficulty() == DUNGEON_HEROIC)
    {
        plr->SetDungeonDifficulty(DUNGEON_NORMAL);
        plr->SendDungeonDifficulty();

        Group* grp = plr->GetGroup();
        if (grp != NULL)
            grp->SetDungeonDifficulty(DUNGEON_NORMAL);
    }

    // if it's not a normal / 10men normal then check if we even have this mode
    if (inf->type == INSTANCE_RAID && plr->GetRaidDifficulty() != RAID_10MAN_NORMAL)
    {
        uint32 newtype = 0;
        ...
        if (!inf->HasDifficulty(plr->GetRaidDifficulty()))
        {
            ...
            switch (plr->GetRaidDifficulty())
            {
                ...
                {
                    newtype = RAID_10MAN_NORMAL;
                    break;
                }

                case RAID_25MAN_HEROIC:
                {
                    newtype = RAID_25MAN_NORMAL;
                    break;
                }
            }
    ...
    else
    {
        ...
        else  // this is the case when we enter the normal way (e.g. we enter thru the portal)
        {
            ...
            if (pGroup != NULL) // we are in a group
            {
                ...
                if ((inf->type == INSTANCE_MULTIMODE && grpdiff == DUNGEON_HEROIC) || inf->type == INSTANCE_RAID)
                {
    ...
    {
        if (inf->type == INSTANCE_MULTIMODE && in->m_difficulty == DUNGEON_HEROIC)
            in->m_expiration = UNIXTIME + TIME_DAY;
        else
            in->m_expiration = (inf->type == INSTANCE_NONRAID || (inf->type == INSTANCE_MULTIMODE && in->m_difficulty == DUNGEON_NORMAL)) ? 0 : UNIXTIME + inf->cooldown;
    }
    else
    {
        if (inf->type == INSTANCE_MULTIMODE && in->m_difficulty >= DUNGEON_HEROIC)
    ...
    for (i = 0; i < NUM_MAPS; ++i)
    {
        ...
        if (instancemap)
        {
            for (itr = instancemap->begin(); itr != instancemap->end();)
            {
                ...
                if (in->m_mapInfo->type != INSTANCE_NONRAID && !(in->m_mapInfo->type == INSTANCE_MULTIMODE && in->m_difficulty == DUNGEON_NORMAL) && HasInstanceExpired(in))
                    _DeleteInstance(in, false);
            }
