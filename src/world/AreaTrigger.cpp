...
uint32 CheckTriggerPrerequisites(AreaTrigger* pAreaTrigger, WorldSession* pSession, Player* pPlayer, MapInfo* pMapInfo)
{
    ...
    if (pPlayer->iInstanceType >= DUNGEON_HEROIC && pMapInfo->type != INSTANCE_MULTIMODE && pMapInfo->type != INSTANCE_NULL)
    ...
    if ((pMapInfo->type == INSTANCE_MULTIMODE && pPlayer->iInstanceType >= DUNGEON_HEROIC) && !pPlayer->GetGroup())
    ...
    if (pPlayer->iInstanceType >= DUNGEON_HEROIC &&
    ...
    if (pMapInfo->type != INSTANCE_NULL && pPlayer->iInstanceType >= DUNGEON_HEROIC && pPlayer->getLevel() < pMapInfo->minlevel_heroic)
