...
class HungerforBloodSpell : public Spell
{
	SPELL_FACTORY_FUNCTION(HungerforBloodSpell);
	...
	void HandleEffects(uint64 guid, uint32 i)
	{
		...
		if (p_caster != NULL)
			p_caster->CastSpell(p_caster, 63848, false); // Hunger for Blood.
	}
...
{
	AddSpellById(51662, &HungerforBloodSpell::Create);
